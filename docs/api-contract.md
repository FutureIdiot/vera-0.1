...
Agent = 实际执行者。稳定身份、私有Memory及Skills / Hooks / MCP / Data配置按`agentId`归属；provider/runtime/model由该Agent daemon登记。Agent不拥有Space、Workspace或Account项目数据，也不得因登录Account读取其默认Agent的Memory。
...
Account = Space中的持久对外身份 + Space/项目数据与Workspace权限边界。User先创建Account，再生成接入Key让Agent代表它上线。

```json
{
  "id": "acc_a1b2",
  "name": "GLM",
  "defaultAgentId": "agt_glm",
  "presence": "offline",
...
- `defaultAgentId`：该Account默认由哪个Agent上线。首个成功接入时若为空则原子写入；之后代上线不修改，只有User可显式更换。
- `activeAgentId`：当前实际代表该Account在线的Agent，来自Account登录会话；离线为`null`，不作为所有权。
- Account不保存`kind/provider/model/connection`。这些属于实际Agent runtime；消息展示使用Execution冻结的`effectiveModel`。
- Account access key是可轮换、可撤销的能力凭证。公开`accessKeyState`只允许`active/revoked`，`accessKeyVersion`每次创建、轮换或撤销都单调递增；gateway仅在active时保存salted hash，创建或轮换响应中明文只返回一次，普通GET、日志和SSE永不返回明文/hash。
- 普通登录必须同时证明agent token与Account access key。唯一例外是首次`enroll`：Account key可创建一个全新的Agent身份并一次性换取其agent token，但不能指定或冒充既有`agentId`；之后只凭Account key不得登录，只凭agent token也不得读取Account的Space/Workspace/Files。
- **单活跃Account会话与Execution租约** `[P5.5]`：同一Account同时只能有一个`activeAgentId`。显式接管先终态化旧在飞Execution并撤销旧会话，再原子切换；非接管竞争返回409 `account_busy`。
- `runtimeCapabilities`是当前`activeAgentId`代表该Account时的临时能力交集快照，离线为`null`；真实Agent能力仍以Agent runtime报告为准。

**Phase 4/5遗留迁移**：现有`owningAgentId/kind/provider/connection/model/authorizedAgentIds`和`Agent 1:N Account`界面都是待删除旧形态。Phase 5.5必须从存量记录一次生成Account、Agent runtime迁移候选和Account access key待轮换状态，改为Account-first创建、`seat.accountId`及双凭证登录；不得长期双读双写或保留Home Account兼容别名。
...
- Execution只能使用其`accountId`对应的Workspace；Agent换Account时明确切换项目边界，但仍只使用自己的Memory。
...
- `SpaceSession 1:N AgentSession`，唯一键`(spaceSessionId,accountId,agentId)`。Account首次由某Agent上线响应前建立；代上线Agent必须建立独立AgentSession，不继承默认Agent的history/binding/Recall sidecar。
...
- Vera持有AgentSession、generation、checkpoint、容量与provider binding元数据真值。API Agent的规范history/checkpoint落gateway store；CLI thread/resume id只作为`(agentSessionId,generation,agentRuntimeFingerprint)`绑定。Account切换或代上线都不得跨Agent复用binding。
...
**Phase 5.5迁移**：现有`(spaceSessionId,agentId)`AgentSession只有在旧seat与旧Account能唯一对应时迁成`(spaceSessionId,accountId,agentId)`；无法唯一对应的provider binding失效并从新generation开始。不得把默认Agent旧thread交给代上线Agent。

**Speaker view 编译层输出契约**：触发某Account当前Agent的Run时，gateway只在当前`spaceSessionId`内，从该`accountId + agentId`组合上次发言之后派生其他Account的署名声告段；`silent/focused/blockAccountIds`统一在此层过滤。prompt开头必须可信说明实际Agent身份、当前代表的Account、是否代上线及切换原因，但不得注入默认Agent Memory。
...
- `author`：用户为`{"type":"user"}`；Account消息为
  `{"type":"account","accountId":"acc_…","accountNameSnapshot":"GLM","executingAgentId":"agt_codex","effectiveModel":"gpt-5.6-sol","delegated":true}`。
- `delegated`必须在发送时按`executingAgentId !== account.defaultAgentId`计算并冻结。普通时间线展示`Account名 · effectiveModel`；为true时仅模型名使用代上线颜色，Account名样式不变。
- `target`：`{"type":"broadcast"}`或`{"type":"direct","accountIds":["acc_…"]}`。
- `accountNameSnapshot/executingAgentId/effectiveModel/delegated`都持久化，防止以后改名、换默认Agent或换模型导致历史漂移；普通Space UI不额外展示实际Agent名，审计页可读取。
...
- `runtimeRevision/effectiveModel`来自实际Agent runtime且创建Run时冻结；`effectiveModel`必须是非空的实际可展示模型名，不能是`default`、Account名或provider名。daemon提交结果时仍须匹配该revision。`delegated`按该时刻`agentId !== account.defaultAgentId`冻结。
...
- Memory读取与写入目标始终由`agentId`决定；Account切换不得生成、复制或改挂Memory。Workspace、Space与项目数据由`accountId`决定；provider/model/runtime由实际Agent决定。
...
同一Account上的活跃Run串行执行。显式代上线接管还必须先终态化旧`activeAgentId`的在飞Run并撤销旧Account会话；不得让新旧Agent同时代表同一Account发言或操作Workspace。
...
- 状态键为`agentId + accountId + spaceId`；同一Agent代表不同Account时不得互相覆盖。Space时间线按Account展示状态，Agent详情按实际Agent聚合。
...
| DELETE | `/api/agents/:id` | 仅无活跃Account会话、无历史/Memory及无Account `defaultAgentId`引用时允许；不连带删除Account、Space或Workspace |

### Account

| Method | Path | 说明 |
|---|---|---|
| GET | `/api/accounts` | 列表；可按`defaultAgentId`或`activeAgentId`过滤，但Account始终是第一层对象 |
| POST | `/api/accounts` | User创建Account。body严格为`{name}`；响应`{account,accessKey}`，其中明文`accessKey`仅此次返回。`defaultAgentId/activeAgentId`初始均为null |
| GET | `/api/accounts/:id` | 返回Account安全详情、Workspace摘要、默认/当前Agent摘要和最近接管审计；不返回Key/hash、Agent Memory或provider secret |
| PATCH | `/api/accounts/:id` | 更新`name/defaultAgentId`。改默认Agent不自动踢当前代上线Agent；需另走接管/撤销动作 |
| POST | `/api/accounts/:id/access-key/rotate` | User轮换Account Key并返回一次性明文；旧Key及由它建立的待重连会话立即失效，在飞Execution按撤销流程终态化 |
| DELETE | `/api/accounts/:id/access-key` | User撤销Account Key且不生成新Key；当前Account会话立即撤销、在飞Execution按撤销流程终态化，Account离线。之后只有显式rotate生成新Key才能再次enroll/login |
| DELETE | `/api/accounts/:id` | 仅无活跃会话/Execution且已按Space删除预检解除Seat/项目引用时允许；不得删除任何Agent Memory |

Vera全局Settings中的`#/settings/accounts`以Account为首层：新建Account、显示默认/当前Agent、生成/轮换Key、Workspace和接管审计。不得出现“新建Agent后自动派生Account”或“添加第二条连接”动作。Agent使用管理仍是独立的`#/agents/:agentId`。
...
| POST | `/api/spaces/:id/session/_compact` `[P5-C1]` | body严格为`{requestId}`；每个Account seat按其当前`accountId + activeAgentId`独立compact并取得Account租约。代上线前后的AgentSession不得混压或共享binding |
...
| Method | Path | 说明 |
|---|---|---|
| POST | `/api/agent/enroll` | 首次Agent登记。`Authorization: Bearer <account-access-key>`，body`{accountId,agent:{name},runtimeProfile}`；成功创建Agent、一次性返回agent token，并在Account尚无默认Agent时设为`defaultAgentId` |
| POST | `/api/agent/login` | `Authorization: Bearer <agent-token>`且`X-Vera-Account-Key`携带Account Key，body`{accountId,takeover?,reason?,runtime,workspace}`。双凭证通过后建立该pair会话；当实际Agent不是`defaultAgentId`或`takeover:true`时，`reason`必须是非空安全文本 |
| DELETE | `/api/agent/sessions/:accountId` | 当前Agent退出指定Account；gateway释放租约并把该Account置offline，保留AgentSession历史、Workspace与默认Agent |
...
| POST | `/api/agent/runs/:id/subagents` | 当前Run请求派生isolated Execution。默认沿用父Run的`agentId + accountId`；若指定其他Account，必须存在该Agent对目标Account的当前有效登录会话，不能靠`authorizedAgentIds`或父Account Key隐式借用 |
...
Space/Agent设置由各自API管理；响应规则per-account per-Space挂在seat上（`responseMode/respondTo/blockAccountIds`）。Account Key、默认/当前Agent和Workspace只在Account系统管理。
...
- Phase 5进程内任务仍暂从旧Account字段解析provider/model；Phase 5.5必须迁移到Agent runtime profile后删除该过渡解析，不保留Home Account兼容层。
...
| GET | `/api/agents/:agentId/memory/_options` | 每类任务列可选执行Agent及其当前runtime revision下真实可用、已验证模型；Digest与Dream资格不复用。不返回Account、secret、宿主路径或夹具正文 |
...
| `run.requested` `[Phase 5.5]` | 公共外壳`{run,triggerMessage,agent,account,workspace,delegationContext,input}`；CLI `input={kind:"cli",sessionMode:"main"|"isolated",promptText,providerBinding?}`，API `input={kind:"api",sessionMode:"main"|"isolated",messages,historyVersion?}` | main Run冻结`agentId/accountId/effectiveModel/spaceSessionId/agentSessionId/contextGeneration/runtimeRevision`；isolated subagent的session字段为null且不带持久binding/historyVersion。`delegationContext`可信说明当前Account、默认/实际Agent、是否代上线与切换原因，不包含默认Agent Memory。CLI只续当前generation绑定；API messages由gateway按容量裁好，daemon不得另存canonical history或回头读store。两种input互斥 |
...
- **SSE事件输入**：`account.upserted/account.presence.updated/agent.updated`，用于默认/当前Agent摘要。
- **API读取**：Account列表与`GET /api/accounts/:accountId`详情。
- **API写入**：`POST /api/accounts`新建、PATCH名称/默认Agent、Key轮换、DELETE。页面不提供新建空Agent、provider/model表单或“添加连接”。
- **页面流程**：新建Account后立即显示一次性接入Key；首个Agent接入后显示默认Agent。User可复制Key给其他已登记Agent代上线，详情页显示当前Agent与最近接管原因。
- **空错态**：无Account→“新建Account”；未接入→“等待Agent使用Key上线”；离线保留默认Agent；Key明文关闭后不可再次读取，只能轮换。

### Agent使用管理 `#/agents`、`#/agents/:agentId`

- **入口与顺序**：Account详情中的默认/当前Agent及消息模型名可进入`#/agents/:agentId`；Account头像不进入Agent页。
