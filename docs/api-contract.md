...
`code` 枚举：`invalid_request`(400) / `unauthorized`(401) / `forbidden`(403) / `not_found`(404) / `conflict`(409) / `account_busy`(409) / `adapter_unavailable`(502) / `internal`(500)。`account_busy` 专指目标 Account 的活跃 Execution 租约已被占用；不得用泛化的 `conflict` 隐去这一原因。
...
### Memory（最小闭环，2026-07-03 提前进场；F1 加 agent 作用域）

全量设计见 `memory-hook.md`（R1–R6，Phase 5）；本轮只落地下述最小闭环，**其余 `/api/agents/:agentId/memory/*`（检索注入、fetch_more / fetch_detail、write hook、dream、派生权重）继续冻结**，形状届时先补本文档。
...
**常驻索引注入**：gateway memory 模块扫描该 agent 的 vault 子目录（`<vaultPath>/<agentId>/`），生成至多 N 行（配置，默认 25）`[[slug]] — 钩子行` 索引，在 (agent, Space) 外部会话的**首条消息**头部注入，附 vault 路径和一句「相关时用你的文件工具展开 [[slug]]」。索引**批量换版**：只随新会话换代，不逐条消息刷新（缓存纪律——它属于稳定前缀，不是逐条变化的检索注入）。MVP 期挑选规则 = 按 `updatedAt` 降序截断，archived 排除；权重排序 Phase 5 再来。

**读写与单写者** `[Phase 5]`：读取仍可由获授权的 CLI 文件工具直接读取该 Agent 的 vault 子目录；**所有程序化写入**（主 Agent、subagent、CLI、daemon、前端和 dream/write hook）必须提交 gateway，由 gateway 按 Agent 串行排队、校验 frontmatter/slug/来源并原子落盘。CLI/daemon 不得直接创建、覆盖、移动或删除 vault 文件。用户通过 Obsidian 手工编辑属于外部输入，gateway 观察到变更后重新校验并更新索引，不把它伪装成程序写入。该单写者在 M1/write hook 实施并验证前仅是目标契约，当前代码可能仍允许旧直写路径。

**Agent 作用域**（F1 修订）：所有 Memory API 路由以 `/api/agents/:agentId/memory` 为前缀，gateway 按 `agentId` 隔离文件读写——agent A 无法读写 agent B 的记忆。前端 Account 管理页进入 `#/settings/accounts/:agentId/memory` 子路由时才加载该 agent 的记忆列表，不预取所有 agent 的 Memory 正文。
...
| Method | Path | 说明 |
|---|---|---|
| GET | `/api/agents/:agentId/memory` | 该 agent 的索引列表 `{ "memories": [{ slug, type, description, status, stains, createdAt, updatedAt }] }`，按 updatedAt 降序。给前端记忆库用，允许含 stains（前端可显示，R5）。agent 不存在 → 404 |
| POST | `/api/agents/:agentId/memory` | 手动「保存到记忆」。body `{ "slug", "type", "description", "content", "stains"? }`；gateway 落盘为上述文件格式到 `<vaultPath>/<agentId>/`。成功 → 201 `{ "memory": { slug, type, description, status, stains, createdAt, updatedAt } }`（与 GET 列表条目同形）；slug 已存在（在该 agent 目录下）→ 409 `conflict`；slug 不合法（非 kebab-case）→ 400；agent 不存在 → 404 |
| GET | `/api/agents/:agentId/memory/:slug` `[P4.6/F1]` | 取单条完整正文 `{ "memory": { slug, type, description, status, stains, createdAt, updatedAt, content } }`；slug 不存在 → 404 |
| PATCH | `/api/agents/:agentId/memory/:slug` `[P4.6/F1]` | 编辑已有记忆。body 任一可选 `{ "type", "description", "status", "content", "stains", "ifMatch" }`：`status` 取 `active` / `archived`（归档不删除，文件保留）；slug 是公共指针，一经建立不可通过普通 PATCH 重命名，任何未知字段 → 400 `invalid_request`。`ifMatch`（ISO updatedAt）做乐观并发控制，与磁盘 `updatedAt` 不一致 → 409 `conflict` 并附 `{ current: {...} }` 让前端重新加载。成功 → 200 `{ "memory": {...} }`，`updatedAt` 由 gateway 重写。slug 不存在 → 404 |
| DELETE | `/api/agents/:agentId/memory/:slug` `[P4.6/F1]` | 删除记忆文件。**不可逆**，前端必须二次确认。成功 → 204；slug 不存在 → 404。删除后历史时间线中已注入的索引不受追溯影响（索引是当时快照） |
...
- **API 读取**：`GET /api/agents/:agentId/memory`（该 agent 的索引列表，按 `updatedAt` 降序，含 stains）；`GET /api/agents/:agentId/memory/:slug`（选中条目按需拉取完整正文）。
- **API 写入**：`POST /api/agents/:agentId/memory`（新建；slug 在该 agent 目录下已存在 409）；`PATCH /api/agents/:agentId/memory/:slug`（只编辑 frontmatter 可变字段 / 正文，slug 不可普通重命名）；`DELETE /api/agents/:agentId/memory/:slug`（删除文件；删除前二次确认）。所有动作进入 gateway per-Agent 单写队列。
- **空错态**：agent 不存在（404）→ 返回 Account 列表 + toast；vault 子目录不存在 / 空 → 「这个 Agent 还没有记忆，用一次就慢慢攒起来了」+ 「手动保存一条」CTA；slug 不存在（404）→ 返回列表 + toast；编辑冲突（同时被 agent 写）→ 加载时记录 `updatedAt`，PATCH 携带 `ifMatch` 校验，不一致返回 409，前端提示「这条刚被 agent 改过，重新加载」；删除二次确认 → 弹层。
