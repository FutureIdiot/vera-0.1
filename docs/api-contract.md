...
| M3 | `memory_search` | `{ query,...filters }` | 关键词+向量宽召回；按稳定slug去重并合并独立一级方向；以本轮相关性×派生权重×单轮交汇因子排序后按token预算裁剪 |
...
M3的`memory_search`返回项统一叫**召回节点**：它是某一条Memory的可独立理解语义投影，必须足以让Agent判断直接使用或调用`memory_fetch_detail`展开同一条Memory的权威正文；不得只返回无语义slug/分数。沿正文中的`SourceRef`读取Message才是溯源到**来源原文**。派生检索结构统一叫**索引**，不作为纵向内容层名称。

单轮去重状态以当前Agent分区内的稳定slug为节点身份；同一节点只返回、展开和计算token一次，但保留命中它的独立一级方向集合。单轮交汇因子基础值为1，随独立一级方向数单调增加、边际递减且封顶；同一一级方向内的重复路径不重复增益。该因子只进入本轮ranking，不进入Memory frontmatter、长期派生权重或使用统计；正文展开和SourceRef溯源不计为横向方向。精确函数、配置默认值、候选字段和游标中的去重状态必须在P5-M3契约先行步骤冻结后才实现。

阶段顺序固定为M3后M4：M3的ranking形状已经是三项，但在M4真实派生权重落地前，`derivedWeight`必须统一取中性值1，不得以`updatedAt`或其他临时启发式冒充。M4接入可重建派生权重时只替换该输入，不改变M3的节点、相关性、交汇、去重、游标或token预算语义。
