...
Agent = 实际执行者。稳定身份、私有Memory及Skills / Hooks / MCP / Data配置按`agentId`归属；provider/runtime/model由该Agent daemon登记。Agent不拥有Space、Workspace或Account项目数据，也不得因代上线读取目标Account owner Agent的Memory。
...
Account = 某个固定owner Agent在Space中的持久对外身份 + Space/项目数据与Workspace权限边界。除首次接入前的待绑定状态外，Account与owner Agent严格1:1；其他Agent只能临时代上线。
...
- `activeAgentId`：当前实际代表该Account在线的Agent，来自Account登录会话；离线为`null`。它可暂时指向非owner Agent，但不改变`ownerAgentId`。
...
- 普通登录必须同时证明agent token与Account access key。唯一例外是Account尚未绑定owner时的首次`enroll`：Account key可创建且只创建一个全新的owner Agent身份并一次性换取其agent token，不能指定或冒充既有`agentId`。owner建立后此Account不得再次`enroll`；未登记runtime必须先创建自己的Account，不能用别人的Account key生成第二个Agent。
- **单活跃Account会话与Execution租约** `[P5.5]`：同一Account同时只能有一个`activeAgentId`。显式接管先终态化旧在飞Execution并撤销旧会话，再原子切换；非接管竞争返回409 `account_busy`。
- **单活跃Agent会话** `[P5.5]`：同一Agent同时只能登录一个Account。代上线前必须退出自己的owner Account或上一个代上线Account；不得同时持有多个活跃Account会话。
- `runtimeCapabilities`是当前`activeAgentId`代表该Account时的临时能力交集快照，离线为`null`；真实Agent能力仍以Agent runtime报告为准。

**Phase 4/5遗留迁移**：现有`owningAgentId`一次改名并收紧为不可变`ownerAgentId`；`kind/provider/connection/model/authorizedAgentIds`从Account删除。Phase 5.5必须把`Agent 1:N Account`存量关系收敛为严格1:1，并生成Agent runtime迁移候选和Account access key待轮换状态；无法唯一确定1:1 owner的记录必须明确阻止迁移并要求人工处理，不得复制Memory、静默拆Agent、长期双读双写或保留兼容别名。
...
- `SpaceSession 1:N AgentSession`，唯一键`(spaceSessionId,accountId,agentId)`。Account首次由owner Agent上线响应前建立；代上线Agent必须建立独立AgentSession，不继承owner Agent的history/binding/Recall sidecar。
...
**Phase 5.5迁移**：现有`(spaceSessionId,agentId)`AgentSession只有在旧seat与旧Account能唯一对应时迁成`(spaceSessionId,accountId,agentId)`；无法唯一对应的provider binding失效并从新generation开始。不得把owner Agent旧thread交给代上线Agent。

**Speaker view 编译层输出契约**：触发某Account当前Agent的Run时，gateway只在当前`spaceSessionId`内，从该`accountId + agentId`组合上次发言之后派生其他Account的署名声告段；`silent/focused/blockAccountIds`统一在此层过滤。prompt开头必须可信说明实际Agent身份、当前代表的Account、是否代上线及切换原因，但不得注入owner Agent Memory。
...
- `delegated`必须在发送时按`executingAgentId !== account.ownerAgentId`计算并冻结。普通时间线展示`Account名 · effectiveModel`；为true时仅模型名使用代上线颜色，Account名样式不变。
...
- `runtimeRevision/effectiveModel`来自实际Agent runtime且创建Run时冻结；`effectiveModel`必须是非空的实际可展示模型名，不能是`default`、Account名或provider名。daemon提交结果时仍须匹配该revision。`delegated`按该时刻`agentId !== account.ownerAgentId`冻结。
...
- 状态键为`agentId + accountId + spaceId`；同一Agent先后代表owner Account或临时代上线Account时不得互相覆盖，但不得同时登录多个Account。Space时间线按Account展示状态，Agent详情按实际Agent聚合。
...
| GET | `/api/accounts/:id` | 返回Account安全详情、Workspace摘要、所属/当前Agent摘要和最近接管审计；不返回Key/hash、Agent Memory或provider secret |
...
Vera全局Settings中的`#/settings/accounts`以Account为首层：新建Account、显示所属/当前Agent、生成/轮换Key、Workspace和接管审计。不得出现“新建Agent后自动派生Account”“更换owner”或“添加第二条连接”动作。Agent使用管理仍是独立的`#/agents/:agentId`。
...
| POST | `/api/agent/login` | `Authorization: Bearer <agent-token>`且`X-Vera-Account-Key`携带Account Key，body`{accountId,takeover?,reason?,runtime,workspace}`。双凭证通过后建立该pair会话；同一Agent已有其他Account会话时必须先退出。实际Agent不是`ownerAgentId`或`takeover:true`时，`reason`必须是非空安全文本 |
| DELETE | `/api/agent/sessions/:accountId` | 当前Agent退出指定Account；gateway释放租约并把该Account置offline，保留AgentSession历史、Workspace与所属Agent |
...
| `run.requested` `[Phase 5.5]` | 公共外壳`{run,triggerMessage,agent,account,workspace,delegationContext,input}`；CLI `input={kind:"cli",sessionMode:"main"|"isolated",promptText,providerBinding?}`，API `input={kind:"api",sessionMode:"main"|"isolated",messages,historyVersion?}` | main Run冻结`agentId/accountId/effectiveModel/spaceSessionId/agentSessionId/contextGeneration/runtimeRevision`；isolated subagent的session字段为null且不带持久binding/historyVersion。`delegationContext`可信说明当前Account、owner/实际Agent、是否代上线与切换原因，不包含owner Agent Memory。CLI只续当前generation绑定；API messages由gateway按容量裁好，daemon不得另存canonical history或回头读store。两种input互斥 |
...
- **页面流程**：新建Account后立即显示一次性接入Key；首次接入原子创建并显示固定owner Agent。User可把Key临时交给其他已登记Agent代上线，详情页显示所属Agent、当前Agent与最近接管原因。
