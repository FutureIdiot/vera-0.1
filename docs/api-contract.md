...
`code` 枚举：`invalid_request`(400) / `unauthorized`(401) / `forbidden`(403) / `not_found`(404) / `conflict`(409) / `adapter_unavailable`(502) / `internal`(500)。
...
**文件库**：默认 `~/.vera/memory/`（配置项，env `VERA_MEMORY_VAULT_PATH`），Obsidian 兼容 vault，在仓库外。**记忆随 agent 身份走**（ground-truth 3.1「私有，随身份走」）：vault 根目录下按 agentId 分子目录 `~/.vera/memory/<agentId>/`，每个 agent 的记忆隔离，slug 在 agent 内唯一。全局/共享记忆（记忆整理产物）在 Phase 5 落地时再定存放规则。每条记忆一个 `.md` 文件：
...
**读写**：读取不限——CLI 型 agent 用自身文件工具直接读 vault 子目录。MVP 期 agent 也可按本格式直接写文件（R1 单写者排队在 write hook 进场时收紧）。
...
- **API 写入**：`POST /api/agents/:agentId/memory`（新建；slug 在该 agent 目录下已存在 409）；`PATCH /api/agents/:agentId/memory/:slug`（编辑 frontmatter 字段 / 正文；可选 `newSlug` 重命名，重命名是文件 mv + 双链更新）；`DELETE /api/agents/:agentId/memory/:slug`（删除文件；删除前二次确认）。
