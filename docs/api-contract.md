...
全量设计见 `memory-hook.md`（R1–R6）。本节只开放 M1 权威文件、来源校验、单写队列、外部重扫和可重建派生索引；write hook/job/proposal（M2），retrieve/fetch_more/fetch_detail、预算、使用统计和 session 去重（M3），权重、dream、merge/rename 维护事务（M4）继续冻结，不得从本节推导公网端点或 worker。
...
**读写与单写者**：读取仍可由获授权的 CLI 文件工具直接读取该 Agent 的 vault 子目录；**所有程序化写入**（主 Agent、subagent、CLI、daemon、前端和未来 hook/dream）必须提交 gateway。现有 POST/PATCH/DELETE 先翻译成内部 `MemoryOperation`：`{ operationId, agentId, origin, kind, slug, ifMatch, value|patch, requestedAt }`；`origin` 在 M1 只有 `user-api` / `external-scan`，`kind` 只有 `create` / `update` / `archive` / `delete`。它不是新公网 API，也不预定义 M2 proposal/job。
