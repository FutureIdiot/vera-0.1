...
- **API 读取**：`GET /api/bootstrap`（列表页用 agents + accounts 组合）；`GET /api/accounts?agentId=…`（详情页按需刷新该 Agent 的唯一 Home Account）；`GET /api/agent-states?agentId=…`（详情页 Agent 当前工作相）。可供 subagent 使用的其他 Account 从 `authorizedAgentIds` 显式派生并标成“已授权”，不得显示成该 Agent 名下 Account。当前内置MCP/Hook读取上文unit binding；Phase 5.5/6第三方unit再按manifest返回自身所需的安全控件。普通能力目录不返回Account连接、provider/model、secret或宿主路径。**不预取Memory配置或正文**——Data → Memory与长期Memory管理均按子路由加载。
...
- **Agent能力与Data目录 `[P5前端与内置binding；Phase 5.5/6通用runtime]`**：Agent组合设置固定提供Skills / Hooks / MCP / Data四个平级入口。Hooks默认内置无执行Agent/模型的Recall/Write；MCP默认内置`vera.memory`，只显示启用状态、可用性与工具清单，不出现semantic Agent或模型。Agent Plugin不作为第五入口且不混入四目录。Digest/Dream配置只在Data → Memory。
- **Agent能力路由与首轮前端接口**：四入口分别进入`#/settings/accounts/:agentId/skills|hooks|mcp|data`。Skills / Hooks / MCP复用上文标准化目录投影和同一个无HTTP view；顶栏右侧固定“添加”“管理”。首轮纯前端交付只验收路由、Shell动作槽、空态、loading/error和夹具列表行，生产Skills为空且所有未接通动作disabled。该交付通过后，Hooks/MCP controller才读取/修改`unit-bindings`；Skills继续为空，直到真实Skill接口完成。Data目录当前只列Memory并按需进入子路由，不使用unit binding。
...
### Agent Memory管理 `#/settings/accounts/:agentId/data/memory/library`
