...
| `run.requested` `[Phase 5.5]` | 公共外壳`{run,triggerMessage,agent,account,workspace,input,activityVisibility?}`；CLI `input={kind:"cli",sessionMode:"main"|"isolated",promptText,providerBinding?}`，API `input={kind:"api",sessionMode:"main"|"isolated",messages,historyVersion?}` | main Run冻结owner `agentId/accountId/effectiveModel/spaceSessionId/agentSessionId/contextGeneration/runtimeRevision`且`delegated:false`；isolated subagent的session字段为null且不带持久binding/historyVersion。阶段C前不得发送`activityVisibility`；落地后必发`status-only/observed`。CLI只续当前generation绑定；API messages由gateway按容量裁好，daemon不得另存canonical history或回头读store。两种input互斥 |
