...
- `memory.vaultPath`：
  1. validate target → 2. `mkdir -p target` → 3. 把当前 vault 下**所有 agent 子目录**整体移到 target（保留 `<agentId>/` 子目录结构）→ 4. 验证 target 内子目录数与文件数一致 → 5. `PATCH /api/settings` 写 `paths.memoryVaultPath = target` override → 6. **gateway 热替换 memory 模块的 vaultPath**（memory 模块提供 `reopen({ vaultPath })` 方法，paths-routes 调用后立即生效，后续 `listMemories` / `residentIndex` / `getMemory` 全部走新路径）→ 7. 返回 `{ restartRequired: false }`。原 vault 目录留空不删（用户自行清理）。失败任一步回滚：把已搬到 target 的子目录 mv 回原 vault。
- `gateway.dataPath`：
