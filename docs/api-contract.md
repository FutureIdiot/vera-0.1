...
- Secret默认不出现在响应中。唯一例外是契约明确的一次性签发响应：Account Key创建/轮换、首次enroll的Agent Token、Key模式login的Account Session Token；这些响应必须`Cache-Control: no-store`且不得进入日志、SSE或后续GET。API型Agent的provider key存`~/.vera/secrets.json`，接口只引用键名（`secretRef`）。
...
| POST | `/api/agent/workspace/authorize` | 为已创建的owner Execution做权限判定。发送Agent Token + Account Session，body严格为`{accountId,runId,workspaceHostId,runtimeRevision}`；响应只返回安全授权摘要`{execution:{runId,accountId,agentId,workspaceHostId,runtimeRevision}}`，不返回Workspace绝对路径、Key、token或secret。当前不接受非owner，不签发可脱离Control Service复用的长期Workspace凭证 |
