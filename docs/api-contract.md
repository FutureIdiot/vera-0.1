...
- 改provider、daemon默认模型或已接入models改的是Agent runtime；Account在候选内切换当前聊天模型只改Account选择。两者都不得迁移或复制Memory。
...
- Account不保存`kind/provider/connection`，但保存独立于owner归属的当前聊天模型选择`model + modelVersion`。`model`只能取所属Agent已登记`runtimeCapabilities.models`中的精确值；它不是Agent身份或能力事实，也不得用于反推或改写`ownerAgentId`。消息展示使用Execution冻结的`effectiveModel`。
...
- `runtimeCapabilities`是owner Agent runtime与其Workspace策略的临时能力交集快照，离线为`null`；其中`models`是去重后的非空模型名列表且必须包含runtime默认`model`。gateway在Agent私有runtime snapshot中保留最近一次安全模型列表供离线配置展示，但真实可执行性仍以当前daemon报告为准。
...
**Phase 4/5遗留迁移**：`owningAgentId`已一次改名并收紧为不可变`ownerAgentId`；旧Account上的`kind/provider/connection/model/authorizedAgentIds`整组runtime字段已删除，`Agent 1:N Account`存量关系已收敛为严格1:1。当前重新出现的Account `model`只是受owner Agent候选约束的聊天选择，并带独立`modelVersion`，不是旧runtime兼容字段。迁移预检无法唯一确定owner时必须在写入前阻止启动并要求人工处理；不得复制Memory、静默拆Agent、长期双读双写或保留兼容别名。
...
- `runtimeRevision`来自owner Agent runtime；`effectiveModel + modelVersion`来自Account当前模型选择且模型必须存在于该Agent当前登记的models中，三者在Run创建时冻结。`effectiveModel`必须是非空实际模型名，不能是`default`、Account名或provider名。daemon提交结果时仍须匹配该revision和冻结模型。Phase 5.5当前`agentId === account.ownerAgentId`且`delegated:false`。
...
- Memory读取与写入目标始终由`agentId`决定；Account切换模型不得生成、复制或改挂Memory。Workspace与项目执行权限由`accountId`决定；Space时间线与附件由`spaceId`决定，Account通过Seat参与；provider/runtime与模型候选由实际Agent决定，Account只决定本次从候选中选择哪个聊天模型。
...
provider明确允许公开的安全过程摘要、工具执行记录等过程信息。Activity不是模型原始隐藏推理的导出通道；隐藏推理不得进入请求、store、SSE或日志。Activity **不是独立面板，是 Space 时间线的正式成员**；只有下述阶段C关注条件成立时，daemon过程Activity才与Message按时间穿插并保留历史。
...
- `phase`：`thinking`（provider可公开的安全过程摘要，不是隐藏推理）/ `tool`（工具执行）/ `working` / `usage` / `error`。
- 工具类 activity 是**同一条记录原地更新**（pending → running → completed/error），不是每个状态一条。
- `detail` 截断长度、activity 历史保留策略均为配置项。

**过程可见性 `[阶段C]`**：gateway维护不持久化的全局`{observedSpaceId,revision}`，进程启动固定为`{observedSpaceId:null,revision:0}`。`observedSpaceId`只能为`null`或一个active且`seats.length===1`的Space id；因此整个部署最多一个被关注私聊，群聊永远是`status-only`。普通bootstrap返回该安全状态，多端修改使用revision CAS并广播`observation.updated`。切换只影响在飞Run此后的Activity，不补发此前过程。

- `activityVisibility`只允许`status-only/observed`。gateway在`run.requested`中下发当前快照，并在关注变化时只向受影响的在飞Run所属daemon发送`run.activity-visibility.updated`。
- daemon在`status-only`时不得调用Activity端点；provider过程只在本机丢弃或合并为非持久AgentState。gateway以当前观察状态为准二次校验，非关注Run或任意群聊Run上报`thinking/tool/working/usage`固定返回403 `forbidden`且不创建Activity、不发布SSE。
- Approval、取消、安全错误终态、最终Message及API规范history所需的安全tool transcript不是Activity可见性旁路，继续按各自契约处理。gateway自身为离线、撤销等安全状态创建的`error` Activity可以保留，但不得携带被抑制的provider过程。
- 阶段C只约束新Activity写入；既有历史Activity继续按原SpaceSession只读展示，不因启用关注语义自动删除、改写或补迁移。需要清理历史时必须另走显式删除/保留方案。
...
- `detail`只能是粗粒度安全工作摘要，不得包含隐藏推理、tool参数/输出、secret、绝对路径或provider原文；gateway执行有界字符串校验，不能用AgentState绕过Activity抑制。
- AgentState是运行时派生状态，不持久化；与Account presence正交：presence说明哪个Agent当前代表Account在线，AgentState说明该pair在某Space内的工作相。同一三元键原地覆盖，完全相同更新零SSE；status变化立即发布，同status仅detail变化至多每`config.agentState.detailUpdateMinIntervalMs`（默认5000ms）发布一次。
...
| GET | `/api/bootstrap` | 一次拉齐聊天主页所需摘要：accounts + agents安全摘要 + spaces + agentStates + 当前SSE `seq`；阶段C再加入`observation:{observedSpaceId,revision}`。联系人栏由Account与Space seats派生 |
| PUT | `/api/observation` `[阶段C]` | body严格为`{spaceId:string|null,ifRevision:number}`；只接受active且恰有一个Account seat的Space，`null`取消关注。revision不匹配返回409 `conflict`且不静默覆盖其他设备刚完成的切换；成功返回`{observation:{observedSpaceId,revision}}`。被关注Space归档、删除或成员改成多个Account时由同一Space事务原子清空并递增revision；`/new`不清空 |
...
| GET | `/api/accounts/:id` | 严格返回`{account,ownerAgent,activeAgent,modelOptions,recentLogins}`。`modelOptions`只含所属Agent最近登记的去重模型名字符串；未接入为`[]`。`account.workspace`只允许安全摘要；不返回Workspace path/policy、Key/hash、Token/fingerprint、boot id、原始身份头、Agent Memory、provider secret或连接配置 |
| PATCH | `/api/accounts/:id` | 只更新`name`；`ownerAgentId`建立后不可由普通API改绑 |
| PUT | `/api/accounts/:id/model` | body严格为`{model,ifVersion}`。模型必须来自该Account所属Agent的`modelOptions`；版本不符409，Agent未接入该模型409 `model_unavailable`。有该Account的pending/running Run或compact时409 `account_busy`。成功递增`modelVersion`、轮换该Account全部active AgentSession generation并清空新generation的CLI binding/API history；不改变owner、Memory、Workspace或历史Message |
...
| GET | `/api/agent/events` | 要求Agent Token + Account Session Token。daemon单一SSE长连接收 *(1)* `agent.heartbeat`，*(2)* 分型`run.requested`：CLI只含`input:{kind:"cli",promptText,providerBinding?}`，API只含`input:{kind:"api",messages,historyVersion}`，subagent的input另标`sessionMode:"isolated"`且无持久binding/history；阶段C再含`activityVisibility`并可接收`run.activity-visibility.updated`，*(3)* CLI/API compact所需的`agent-session.compact.requested`，*(4)* 配置变更。普通断线以同一Session Token重连，不要求Account Key；不得同时发送两种input或在API payload夹带`promptText/providerBinding` |
...
| POST | `/api/agent/runs/:id/messages` | daemon提交本Run当前回复的权威完整`content`（可带安全`target/fileIds`）。已有delta时本请求只定稿gateway当前气泡，不重复追加正文；零delta时以完整content创建并定稿fallback气泡。gateway从Run冻结`accountId/accountNameSnapshot/agentId/effectiveModel/delegated`生成author并返回权威Message id；daemon不得自造id/status/author |
| POST | `/api/agent/runs/:id/delta` | 当前气泡的流式增量。gateway 转 `message.delta` SSE 事件给前端 |
| POST | `/api/agent/runs/:id/activities` | 创建/更新 activity（带 `callId` 合并同一条），落地 + `activity.created`/`activity.updated` SSE。阶段C逐请求复验当前关注私聊与Run；非关注或群聊provider过程固定403且零写入 |
| POST | `/api/agent/runs/:id/approvals` | 提权申请，gateway 转 `approval.requested` 给前端。prompt只能是安全操作摘要，不得原样携带secret、绝对路径、完整命令或provider原文；前端需要的选择仍由结构化options表达 |
...
| GET | `/api/agent/memory-tasks/events` `[Phase 5.5 internal]` | 只要求Agent Token，不要求也不接受Account Key或Account Session。每个已认证Agent一条Memory worker SSE，接收`memory-task.requested`、`memory-task.cancelled`与`agent.heartbeat`；broker只能把任务投给冻结`executorAgentId`等于Token Agent的连接。该通道不得承载Account、Run、AgentSession或聊天事件 |
| PUT | `/api/agent/memory-tasks/:dispatchId/result` `[Phase 5.5 internal]` | 只要求Agent Token。body严格为`{attempt,status:"succeeded"|"failed"|"cancelled",proposals?,execution?,error?}`；gateway以`dispatchId + attempt + Token Agent`匹配在途任务，提交前重验冻结runtime/task model。完全相同重试幂等，错误Agent、旧attempt、迟到或不同结果拒绝。成功`execution`只能含冻结`runtimeRevision/taskModel/fallbackUsed:false`；不得返回provider原文、secret、路径或日志 |
...
Memory task请求事件严格为`{dispatchId,jobId,attempt,kind,memoryTaskSnapshot,payload}`。出线的`memoryTaskSnapshot`只含`{ownerAgentId,executorAgentId,runtimeRevision,kind,provider,modelMode,taskModel,verificationId}`，不得包含gateway私有`connectionFingerprint`、runtime connection、Account、Workspace、AgentSession、provider binding、system prompt、Tools或secret。`payload`只取当前Digest/Dream proposal executor已经裁剪的任务输入：Digest为`agent/chunks/facts/proposalSchema`，Dream为`agent/memories/proposalSchema`；允许任务所必需的owner Memory正文，但不得夹带executor Memory或完整聊天/Activity历史。迁移期通过显式transport选择进程内或daemon执行；一旦某任务冻结为daemon，缺席、断线、超时或revision/model漂移必须安全失败，不得在同一任务上自动回退进程内adapter。

### Owner Tailscale 身份 `[Phase 5.5]`

普通客户端不使用Vera自建配对码或device session。这里不包含daemon ↔ gateway之间的进程内Account Session Token；后者只授权Agent访问自己的Account，不代表owner客户端设备。Tailscale Serve必须覆盖/清除客户端伪造的同名身份头后，把已认证login写入`Tailscale-User-Login`并转发给回环gateway。gateway只在`req.socket.remoteAddress`是`127.0.0.1`、`::1`或`::ffff:127.0.0.1`时读取该header；其他来源即使携带同名header也一律视为未认证，不信任`X-Forwarded-For`或其他客户端可写来源头。本边界信任VPS本机进程与Tailscale Serve；生产部署仍必须按第五节只监听`127.0.0.1:3210`，不能把“检查remoteAddress”当作允许公网监听的替代。

部署配置固定为：

- `config.security.ownerTailscaleLogins`默认`[]`，env override为`VERA_OWNER_TAILSCALE_LOGINS`；env是逗号分隔列表，逐项trim、丢弃空项并按trim后的完整字符串去重。login使用大小写敏感的完整字符串精确匹配；不得转小写、做域名后缀/前缀匹配或接受通配符。
- `config.security.cors.allowedOrigins`默认`[]`，env override为`VERA_CORS_ALLOWED_ORIGINS`；同样按逗号分隔、trim、丢弃空项并精确去重。每项必须是单一序列化Origin，不得带path/query/fragment；`*`与`null`不是合法配置值。
- `config.security.allowLoopbackDevelopment`默认`false`，env override为`VERA_ALLOW_LOOPBACK_DEVELOPMENT`；只有大小写不敏感的`true`启用，空值、`false`及其他值均为false。该豁免只允许上述loopback来源在没有`Tailscale-User-Login`时访问，用于本地开发与测试；它不信任非loopback来源，也不把伪造header转成owner身份。`NODE_ENV=production`时该值为true属于启动配置错误，gateway不得开始监听。

入口授权矩阵固定如下：

| 请求 | Owner身份要求 |
|---|---|
| `GET /api/health` | 最小存活探针，豁免Owner身份与Agent凭证；不得因此扩展其他health写入或数据读取 |
| `/api/agent/*` | 不读取、不匹配也不记录Owner identity；实际业务请求继续只按本节前述Agent Token、Account Key / Account Session规则授权，Owner身份不得旁路任一Agent凭证 |
| 其余`/api/*`（含`/api/events`） | 必须由可信loopback Serve来源提供精确命中的Owner login，或命中显式development loopback豁免 |
| 前端HTML、静态资源与SPA fallback | 与普通Owner API相同；未认证请求不能只因“静态文件无secret”而取得页面入口 |

`ownerTailscaleLogins`为空、可信header缺失或login不匹配时，上述普通入口统一返回不含身份值的403 `forbidden`；生产空列表还必须写一条安全配置错误，但日志、Status、API错误、SSE及持久化数据均不得包含原始`Tailscale-User-Login`、allowlist内容或其派生可识别值。设备加入、过期与撤销由tailnet管理；Vera不复制设备目录，也不建立第二套配对码或device session。

**精确Origin CORS边界**：缺少`Origin`的daemon、CLI与同源导航按非CORS请求继续走上表身份/凭证授权。携带`Origin`时，只接受与请求有效同源Origin完全相同的值，或精确命中`config.security.cors.allowedOrigins`的值；生产Serve请求的有效同源Origin固定按`https://`加原始`Host`计算，development loopback豁免请求按实际`http://`或`https://`加`Host`计算。不得用子域、前缀、后缀、正则或规范化后的近似值放宽匹配。其他Origin及其预检统一返回不含CORS授权头的403，路由不得执行。

命中跨域allowlist时，响应只把请求`Origin`原值写入`Access-Control-Allow-Origin`；禁止`*`，并始终省略`Access-Control-Allow-Credentials`。动态Origin响应（包括允许的预检、普通响应与安全错误响应）必须把`Origin`合并进既有`Vary`，不得覆盖其他`Vary`值。预检`OPTIONS`只有在Origin通过、`Access-Control-Request-Method`大小写规范化后精确属于`GET, POST, PATCH, DELETE, OPTIONS`，且`Access-Control-Request-Headers`逐项trim、大小写不敏感地全部属于`Authorization, Content-Type, Last-Event-ID`时返回204空响应；否则403。允许的预检返回固定`Access-Control-Allow-Methods: GET, POST, PATCH, DELETE, OPTIONS`与`Access-Control-Allow-Headers: Authorization, Content-Type, Last-Event-ID`。预检204只表示浏览器传输边界允许，不能替代随后实际请求的Owner或Agent授权；特别是`/api/agent/*`不因预检成功而接受Owner身份或缺失Agent凭证。
...
- gateway-local任务只从执行Agent的`runtimeProfile/runtimeBinding`与当前`runtimeRevision`解析provider和模型候选，再使用Account当前`model`选择冻结本次模型；不得读取Phase 4旧Account runtime字段或Home Account兼容层。daemon执行保持同一选择与资格语义。
...
| `activity.created` | `{ activity }` | 获准关注私聊的新安全过程摘要或工具事件；默认/群聊不会产生daemon过程Activity |
| `activity.updated` | `{ activity }` | 同一条目状态/内容更新（工具 pending→completed 等） |
| `observation.updated` `[阶段C]` | `{observation:{observedSpaceId,revision}}` | 全局关注私聊变化；客户端据此同步唯一“关注”状态，不把它写成Space持久字段 |
...
| `run.requested` `[Phase 5.5]` | 公共外壳`{run,triggerMessage,agent,account,workspace,input,activityVisibility?}`；CLI `input={kind:"cli",sessionMode:"main"|"isolated",promptText,providerBinding?}`，API `input={kind:"api",sessionMode:"main"|"isolated",messages,historyVersion?}` | main Run冻结owner `agentId/accountId/effectiveModel/modelVersion/spaceSessionId/agentSessionId/contextGeneration/runtimeRevision`且`delegated:false`；isolated subagent的session字段为null且不带持久binding/historyVersion。阶段C前不得发送`activityVisibility`；落地后必发`status-only/observed`。CLI只续当前generation绑定；API messages由gateway按容量裁好，daemon不得另存canonical history或回头读store。两种input互斥 |
| `run.activity-visibility.updated` `[阶段C daemon-only]` | `{runId,activityVisibility}` | 只发给受关注切换影响且仍在飞Run的对应daemon；只改变后续Activity上报，不改变Execution、prompt、工具权限、Approval或最终Message |
| `stream.reset` | `{}` | 客户端必须重新 bootstrap |

**默认或群聊回复的事件序列 `[阶段C]`**（过程在daemon源头抑制，Message流式语义不变）：

```
run.started
agent.state.updated (on_task)
message.created      (气泡, streaming)
message.delta ×N
message.completed
run.ended            (completed)
```

**被关注私聊回复的事件序列 `[阶段C]`**（安全过程与气泡按实际发生时间穿插）：

```
run.started
activity.created   (thinking：安全过程摘要)
...
approval.requested (请求执行清理操作，等用户点按钮)
...
- **SSE 事件输入**：既有Message/Activity/Approval/Run事件必须同时匹配当前`spaceSessionId`；阶段C的`observation.updated`同步全局唯一关注状态。新增`space-session.archived`、`space-session.created`切换active窗口，`agent-session.compaction.updated`刷新per-Agent generation/压力与手动job结果。`file.deleted`命中当前时间线附件时把链接标成deleted；`file.updated`不直接扩大本地授权，必要时重取时间线。归档历史页不消费写事件。
- **API 读取**：`GET /api/bootstrap`（首屏）；`GET /api/spaces/:id/timeline?before=&limit=50`（向上翻页加载更早历史）。
- **API 写入**：普通正文/附件引用走`POST /api/spaces/:id/messages`；composer先通过Files上传端点取得File id，再随Message提交。精确`/compact`与`/new`分别走`POST .../session/_compact`与`POST .../session/_new`，不落Message；取消Run和Approval回答沿用既有端点。阶段C只有active且恰有一个Account seat的私聊显示“关注/取消关注”，以bootstrap revision调用`PUT /api/observation`。
- **空错态**：Space 不存在（404）→ 主区显示「Space 不存在或已归档」+ 返回导航入口；时间线空 → 「还没有消息，发一条开始」；时间线长 → DOM 上限 200 items，更早走 `?before=` 分页；群聊、已归档Space与归档SpaceSession不显示关注动作；关注revision冲突时重取bootstrap后显示当前真实关注Space，不静默抢占。approval 失效（409）→ 卡片灰化；发送失败（4xx/5xx）→ composer 内联错误 + 保留草稿；SSE 断连 → 时间线冻结 + 顶部「重连中」；已归档 Space 通过此路由进入 → 消息发送返回 409，主区显示「已归档，去设置恢复」入口。
...
- **API写入**：`POST /api/accounts`新建、PATCH名称、`PUT /api/accounts/:id/model`按CAS切换模型、Key轮换、DELETE。页面不提供新建空Agent、owner改绑、provider/connection配置、自由文本模型或“添加连接”。
- **页面流程**：新建Account后立即显示一次性接入Key；首次接入原子创建并显示固定owner Agent，并以该Agent默认模型初始化Account选择。详情页把“所属 Agent”和“模型”显示为两个独立词条；模型用`modelOptions`下拉，失效选择原样显示并阻止新Run，不自动换成列表首项。切换成功只影响后续Run并明确提示聊天上下文已轮换；Workspace、登录审计和active Space仍按既有来源展示，不提供代上线或接管入口。
...
- **CORS**：Web同源请求与原生壳跨域请求统一遵循「Owner Tailscale身份」一节的精确Origin矩阵。原生Origin必须显式出现在`config.security.cors.allowedOrigins`；只对精确命中值回显`Access-Control-Allow-Origin`并合并`Vary: Origin`，禁止`*`与credentials。只有Origin、请求方法和请求头全部通过固定白名单时预检`OPTIONS`才返回204，其他Origin/预检均403且不执行路由。
