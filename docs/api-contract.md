...
`code` 枚举：`invalid_request`(400) / `unauthorized`(401) / `forbidden`(403) / `not_found`(404) / `conflict`(409) / `account_busy`(409) / `invalid_memory_file`(422) / `adapter_unavailable`(502) / `internal`(500)。`account_busy` 专指目标 Account 的活跃 Execution 租约已被占用；不得用泛化的 `conflict` 隐去这一原因。错误对象可带领域专用的 `details`，但不得改变 `code/message` 的通用包络。
...
Agent = Vera 内独立身份实体。身份字段只有命名 + 时间戳；Skills / MCP / Hooks / Agent Plugins的安装与启用是独立的per-Agent扩展绑定，不嵌进Agent身份。连接、供应商、模型、外部会话上下文仍随 Account 走。
...
`[Phase 5.5/6]` MCP/Hook unit binding是独立资源，最小公共语义为`{agentId,unitId,enabled,executorAgentId,version}`。配置中的`executorAgentId:null`表示当前/所属Agent；owner只通过执行Agent下拉修改它，gateway在创建实际任务时解析成明确Agent id。gateway根据unit要求与真实runtime/资格状态生成可用Agent候选，但普通UI不展示“兼容性”字段；已选Agent不可用时保留绑定并返回安全`availability:"unavailable"`供前端显示`!`。启用且执行者不可用时明确不运行，不得自动改投。Skills不使用`executorAgentId`；Extension Package manifest/API及各unit专属配置仍须在Phase 6契约先行步骤冻结后实现。内置unit固定为MCP `vera.memory`与Hook `vera.memory.digest`，名称分别展示为`Vera Memory MCP`和`Vera Memory Digest Hook`。
...
### Memory（P5-M1 权威层、单写者与溯源）

全量设计见 `memory-hook.md`（R1–R6）。本节只开放 M1 权威文件、来源校验、单写队列、外部重扫和可重建派生索引；write hook/job/proposal（M2），search/fetch_more及fetch_detail的M3增强（一跳链接、使用统计）、预算和session去重（M3），权重、dream、merge/rename维护事务（M4）继续冻结，不得从本节推导公网端点或worker。

**文件库**：默认 `~/.vera/memory/`（配置项，env `VERA_MEMORY_VAULT_PATH`），Obsidian 兼容 vault，在仓库外。**长期 Memory 随 agent 身份跨 Space、跨 Account/Execution**：vault 根目录下按 agentId 分子目录 `~/.vera/memory/<agentId>/`，每个 agent 的记忆隔离，slug 在 agent 内唯一且建立后不可普通改名。不存在隐式“所有 Agent 可读”的全局池；未来若需要共享，必须另定义显式作用域、授权和来源契约，不得把 per-Agent 目录合并扫描。每条记忆一个 `.md` 文件：
...
**常驻索引注入**：gateway memory 模块扫描该 agent 的 vault 子目录（`<vaultPath>/<agentId>/`），生成至多 N 行（配置，默认 25）`[[slug]] — 钩子行` 索引，在 (agent, Space) 外部会话的**首条消息**头部注入，并提示「相关时调用Vera Memory MCP展开slug」。不向Agent暴露宿主vault绝对路径，也不让它用文件工具直读。索引**批量换版**：只随新会话换代，不逐条消息刷新（缓存纪律——它属于稳定前缀，不是逐条变化的检索注入）。MVP期挑选规则=按`updatedAt`降序截断，archived排除；权重排序Phase 5再来。

**读写与单写者**：Agent runtime（主Agent、subagent、CLI、daemon及未来hook/dream）的Memory读写统一进入gateway第一方Vera Memory MCP；不得用文件工具直读/直写vault。owner前端保留HTTP管理API。HTTP与MCP都调用同一Memory facade；写入最终翻译成内部`MemoryOperation`：`{ operationId, agentId, origin, kind, slug, ifMatch, value|patch, requestedAt }`。origin为`user-api`/`agent-mcp`/`memory-hook`/`external-scan`，kind为`create`/`update`/`archive`/`delete`。MemoryOperation不是公网API。
...
**Agent 作用域**（F1修订）：owner Memory管理API以`/api/agents/:agentId/memory`为前缀；Vera Memory MCP tool参数中**禁止出现`agentId`**，gateway从可信调用上下文注入身份。两条入口都按`agentId`隔离同一Memory facade——agent A无法读写agent B的记忆。前端Account管理页进入`#/settings/accounts/:agentId/memory`子路由时才加载该agent的记忆列表，不预取所有agent的Memory正文。

**派生索引与坏文件隔离**：索引放在 `<vaultPath>/.vera-index/<agentId>.json`，形状为 `{ schemaVersion:1, generation, builtAt, agentId, entries, errors }`；entry 至少含 `slug/version/type/description/status/sourceRefs/links/updatedAt`。索引只是缓存：缺失、JSON 损坏、版本不支持或文件指纹变化时，只读 vault + store SourceRef 全量重建，经临时文件原子替换索引；不得反向覆盖 markdown。每次 scan 返回 `{ agentId, scannedAt, created, updated, removed, unchangedCount, invalid, index:{ generation, builtAt, status } }`，其中 status 为 `current` / `rebuilt` / `degraded`；索引写失败只令 status=`degraded`，不混进 MemoryFileError。坏文件保留原位、排除列表/常驻索引，并产生 `{ code, relativePath, slug, issues:[{ field, code, message }] }`；对外不得泄露 vault 绝对路径。单个坏文件不影响其他记忆；直接 GET 坏文件返回 422 `invalid_memory_file`。
...
| M3 | `memory_search` | `{ query,...filters }` | 关键词+向量取得出发节点后开放图扩散；归并同slug命中，以相关性/距离、长期权重、单轮置信、类型适配计分，再做语义去重、粒度选择、type软配额重排与token预算裁剪 |
| M3 | `memory_fetch_more` | `{ cursor,direction }` | 稳定游标继续展开候选 |
...
`memory.digestTrigger` 是单选自动策略：`scheduled` 与 `realtime` 不同时运行；选 `manual` 只关闭自动策略。owner HTTP 与可信 Agent MCP 的 manual 提交始终可用。`scheduled` 使用五段 cron，并按 gateway 进程时区解释；gateway启动、从其他策略切入scheduled或cron表达式变化时，对已有未整理窗口立即执行一次catch-up；无关Settings更新只重排timer，不越过cron触发整理。`realtime` 的唯一水位是 `(agentId, spaceId)` 下、上次成功增量 job 之后已完整保存 Message 正文的 Unicode code point 数；阈值为 `memory.digestRealtimeThresholdChars`。它不是 provider token 估算或 adapter context capacity。只计`status=completed`的Message；Activity、流式delta与工具过程不计。达到阈值只异步 enqueue，聊天请求和 Run 结束均不等待整理。Run 结束不等于外部 session 结束；session-end dream 归 M4，M2 不增加兜底开关。
...
持久 job 安全摘要：`{ id,agentId,spaceId,mode,trigger,range:{fromMessageId,toMessageId,messageCount,charCount},pipelineVersion,idempotencyKey,status,attempt,createdAt,startedAt?,finishedAt?,error?,result? }`。status 为 `queued/running/applying/succeeded/failed/cancelled`；error code 只允许 `executor_unavailable/executor_failed/invalid_range/invalid_proposal/write_conflict/write_failed/cancelled`，message为不含provider原文的固定安全文案。当前M2 `idempotencyKey = sha256(agentId|spaceId|from|to|mode|pipelineVersion)`；Memory Digest Hook执行Agent绑定落地时，入队必须先解析并内部冻结`executorSnapshot={executorAgentId,accountId,kind,provider,model,verificationId}`，再把该snapshot的稳定指纹加入幂等键并提升pipelineVersion。snapshot不得含connection/secret/sessionState/Workspace/name，不得出现在safe job、SSE、MCP或普通日志；retry复用原snapshot，配置变化只影响新job，执行Agent/Account删除、不可用或验证撤销后尚未spawn的job明确`executor_unavailable`，不得动态改投。gateway 重启把遗留 running/applying job 重新排队并增加 attempt。每个 proposalId 与 MemoryOperation.operationId 都由 job、序号和规范化 proposal 稳定派生；job内持久applications receipt（pending/applied/noop + slug + before/after version）。重试若目标已经达到同一权威状态则为 no-op，不得重复创建或丢失 SourceRef；pipeline语义变化必须提升pipelineVersion，旧job仍按旧版本只读展示。
...
`memory_digest`的HTTP/MCP输入永远不接受executorAgentId、accountId、provider、model或fallbackModel。当前M2尚未落Hook unit绑定时固定由Memory所属Agent自身执行；绑定落地后由gateway只在入队时解析`Vera Memory Digest Hook`的`executorAgentId`，再取该执行Agent唯一Home Account及已验证model。gateway把最小可信控制参数`account={id,kind,provider,connection,model}`与实际模型`payload`分开传给adapter，后者不得读取unit binding、自行换执行Agent/Account/model或把Account connection/secret送进prompt。`kind=api, provider=ollama`的Gemma Account由原生Ollama adapter直接调用`connection.baseUrl`，使用冻结model、固定digest `temperature=0`并按已实测能力下沉transport schema；`kind=cli, provider=codex`的Codex Account使用冻结model和非交互`codex exec`，聊天首轮从JSONL `thread.started`保存`sessionState={threadId}`，续轮只用`codex exec resume <threadId>`，临时路径不得持久化。Codex digest每个job都在新临时cwd执行`--ephemeral --ignore-user-config --ignore-rules`，不resume或读写聊天sessionState，不传Account Workspace，使用read-only sandbox和`approval_policy=never`，并把gateway给出的完整schema或确定性Codex-compatible投影写入临时文件后强制传`--output-schema <file>`；任何tool JSONL item都使executor失败，输出还须经过gateway完整validator。Ollama与Codex路径不共享Account、sessionState或连接，均不得选择fallback。OpenCode的聊天与digest代码保留，但`provider=opencode`当前不在Memory digest dispatch中，job明确`executor_unavailable`；不得退化调用聊天`run(ctx)`。公共job仍只返回安全状态/错误，不返回executorSnapshot、endpoint、命令、provider原文、宿主路径或secret。

Digest资格不是“adapter存在”或`presence=online`。gateway判断某Agent能否进入`vera.memory.digest`执行Agent候选时，必须使用真实资格夹具生成的不可伪造内部记录，绑定精确Account连接/远程runtime修订、kind/provider、adapter profile与model不可变标识；本地模型的不同tag/量化变体分别验证。Account连接、runtime、provider或model改变使旧资格失效；同一模型搬宿主只可复用语义资格，仍须重新验证容量、超时与性能。这些细节只参与后台`availability`判断，不在普通下拉中展示兼容性、Account、provider或model。失败无当前Agent、其他Agent、其他Account或其他模型fallback。
...
精确归一化区间、五项权重、距离衰减、交汇函数、语义簇阈值、粒度选择、软配额目标/惩罚及封顶、稳定tie-break、候选字段和游标状态必须在P5-M3契约先行步骤冻结后才实现。阶段顺序固定为M3后M4：M3先把`derivedWeight`贡献统一设为不改变相对顺序的加法中性常量0，不得以`updatedAt`或其他临时启发式冒充；M4只替换该输入，不改变M3的召回顺序、其他四项分数、去重、重排、游标或token预算语义。
...
| Method | Path | 说明 |
|---|---|---|
| GET | `/api/agents/:agentId/memory` | 先重扫该 Agent，返回 `{ memories:[{ slug,type,description,status,stains,sourceCount,createdAt,updatedAt,version }], errors:MemoryFileError[], index:{ generation,builtAt,status } }`，memories 按 updatedAt 降序。agent 不存在 → 404 |
...
- **API 读取**：`GET /api/bootstrap`（列表页用 agents + accounts 组合）；`GET /api/accounts?agentId=…`（详情页按需刷新该 Agent 的唯一 Home Account）；`GET /api/agent-states?agentId=…`（详情页 Agent 当前工作相）。可供 subagent 使用的其他 Account 从 `authorizedAgentIds` 显式派生并标成“已授权”，不得显示成该 Agent 名下 Account。Phase 5.5/6的MCP/Hook unit详情另取gateway给出的安全执行Agent候选`{id,name,availability}`；普通UI不返回或展示兼容性、Account、provider/model、connection、secret或宿主路径。**不预取 Memory 正文**——Memory 在 `#/settings/accounts/:agentId/memory` 子路由按需加载。
- **API 写入**：`POST /api/agents`（新建 Agent；响应含自动派生的 Home Account）；`PATCH /api/agents/:id`（只改`name`）；`DELETE /api/agents/:id`（删除身份 + Home Account；有历史、活跃 Execution、其他Agent unit binding或未完成digest job引用时返回409）；`PATCH /api/accounts/:id`（改连接字段或 `authorizedAgentIds`）。不提供“为同一 Agent 新增 Account”或单独删除 Home Account 的 UI 动作。
- **Agent扩展目录 `[Phase 5.5/6]`**：Agent组合设置下分别提供Skills、MCP、Hooks、Agent Plugins入口。MCP/Hook的每个unit行只显示启用开关与执行Agent下拉；默认当前Agent。已选执行Agent不可用时保留选择并显示`!`和“请指定其他可用Agent”，下拉只提供gateway判定可用的替代Agent；不得展示兼容性字段、Account/provider/model自由文本，也不得自动fallback。Memory分别作为内置`Vera Memory MCP`与`Vera Memory Digest Hook`出现。目录中的导入动作必须复用唯一Extension Package安装事实来源，不能复制安装记录或包格式。
...
- **API 读取**：`GET /api/agents/:agentId/memory`（该 agent 的索引列表、坏文件 errors 与索引代际，按 `updatedAt` 降序）；`GET /api/agents/:agentId/memory/:slug`（选中条目按需拉取完整正文、sources 与 opaque version）。
- **API 写入**：`POST /api/agents/:agentId/memory`（新建；slug 在该 agent 目录下已存在 409）；`PATCH /api/agents/:agentId/memory/:slug`（只编辑 frontmatter 可变字段 / 正文，slug 不可普通重命名，携带 `ifMatch=version`）；`DELETE /api/agents/:agentId/memory/:slug?ifMatch=<version>`（删除文件；删除前二次确认）。所有动作进入 gateway per-Agent 单写队列。
...
- `memory.vaultPath`：
  1. 检查根目录未归属 `*.md`（存在则 409）→ 2. validate target → 3. `mkdir -p target` → 4. 把当前 vault 下**所有 agent 子目录**整体移到 target（保留 `<agentId>/` 子目录结构）→ 5. 验证 target 内子目录数与文件数一致 → 6. `PATCH /api/settings` 写 `paths.memoryVaultPath = target` override 并等待落盘 → 7. **gateway 热替换 memory 模块的 vaultPath**（memory 模块提供 `reopen({ vaultPath })` 方法，paths-routes 调用后立即生效，后续 `listMemories` / `residentIndex` / `getMemory` 全部走新路径）→ 8. 返回 `{ restartRequired: false }`。原 vault 目录留空不删（用户自行清理）。失败任一步回滚：恢复 setting override、把已搬到 target 的子目录 mv 回原 vault。
- `gateway.dataPath`：
