...
`code` 枚举：`invalid_request`(400) / `memory_cursor_invalid`(400) / `unauthorized`(401) / `forbidden`(403) / `not_found`(404) / `memory_cursor_expired`(410) / `conflict`(409) / `account_busy`(409) / `invalid_memory_file`(422) / `adapter_unavailable`(502) / `memory_retrieval_unavailable`(503) / `internal`(500)。`account_busy` 专指目标 Account 的活跃 Execution 租约已被占用；不得用泛化的 `conflict` 隐去这一原因。错误对象可带领域专用的 `details`，但不得改变 `code/message` 的通用包络。
