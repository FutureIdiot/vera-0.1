...
原生Ollama/Gemma是另一条独立API Account，不复用上面的OpenCode CLI Account。以下只展示该Account特有的连接字段，公共id、owner和状态字段省略：
...
`memory_digest`的HTTP/MCP输入不接受provider、model或fallbackModel；路由固定取该Agent当前Home Account。gateway把最小可信控制参数`account={id,kind,provider,connection,model}`与实际模型`payload`分开传给adapter，后者不得把Account connection/secret送进prompt。`kind=api, provider=ollama`的Gemma Account由原生Ollama adapter直接调用`connection.baseUrl`，使用`account.model`并按已实测能力下沉transport schema，绝不经过OpenCode；`kind=cli, provider=opencode`的Navy Account使用Account.model创建独立无Tools session。两条路径不共享Account、sessionState、连接或fallback。OpenCode仅在Navy primary返回402/403/429且结构化错误`code/type`精确为`insufficient_quota`、`quota_exhausted`或`quota_exceeded`时允许同一job用运行配置中的免费模型重试一次，不改Account.model/sessionState，也不影响聊天`run(ctx)`或下一job；Ollama失败不得走此fallback，单独的HTTP状态、自由文本、超时、网络、认证、模型不存在、坏结构化输出均不得触发后备。公共job仍只返回安全状态/错误，不返回endpoint、命令、provider原文或secret。
