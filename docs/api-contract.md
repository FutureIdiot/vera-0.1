...
- `GET /api/settings` → 200 `{ settings: <object> }`：返回合并视图（overrides 叠 config 启动默认）。字段清单严格按 ground truth 4.1，运维参数（端口、数据路径、SSE 心跳/缓冲、store 落盘、daemon 回收、run 看门狗）走 env 不进，前端可读可写。
...
**持久化语义**：`<dataPath>/settings.json` 单文件，防抖落盘（与 store 同 200ms 节流）。config.js 仍是启动默认 source（env 派生），settings.json 是运行时覆盖；只 persist overrides，不 persist 默认值。consumer 接入（bubble-stream / view-compiler / memory 整理等）在 Phase 4.6 及以后，本阶段只提供存储 + API + 字段白名单校验。
