...
- [x] **Dream异步维护（2026-07-15）**：持久job只执行严格`keep/update/merge/archive` proposal；整批预检后进入同一Agent单写者，保留sources与组外双链，默认归档不物理删除，失败不阻塞聊天。
