...
| Memory Provider / Digest / Dream | per-Agent `/api/agents/:agentId/memory/_config|_options|_status` | `#/settings/accounts/:agentId/data/memory` | Provider facade / Digest / Dream | M4契约已冻结，后端完成后再接真实页面；旧全局Digest设置只作一次迁移输入 |
...
>
> **默认 Vera Provider 的 vault位置策略**（2026-07-04，与 Theta 确认 + 联邦形态对齐）：vault 热数据**只在 VPS**（`/home/theta/.vera/memory/`，联邦后 gateway 跑在 VPS），所有agent daemon通过gateway第一方Vera Memory MCP远程读写，本地不再有“原版”——避免双写冲突/双读漂移，保持single source of truth。**备份走git镜像**：把VPS vault init成git repo，每次整理后`git push`到私有GitHub repo；Mac上`git pull`只读备份并查看版本历史。rsync冷备份作次要手段，不替代git镜像的版本维度。Phase 3-4期间vault仍在本机Mac，Phase 5.5联邦落地时随数据迁到VPS并接通带agent token的MCP transport。未来自定义Provider可没有本地路径或Markdown文档，其自身位置/连接状态按Provider配置展示，不套用本段vault迁移与git备份规则。
...
- [ ] **M4契约先行（2026-07-15产品语义已确认）**：`vera.memory.recall`在消息上下文编译前执行自动检索+预算内注入；`vera.memory.write`在completed Message保存后观察待整理水位，并按Data → Memory的`realtime` / `scheduled`自动策略请求Memory Orchestrator创建Message范围Digest job，owner的`manual`请求绕过自动Hook开关。两项Hook均由gateway程序执行，不选择执行Agent或任务模型；关闭Recall只停止自动注入，MCP主动读仍可；关闭Write只停止自动Digest，Message仍保存，`pendingContext`仍可由已保存Message与Digest watermark计算，owner手动Digest与MCP手动保存仍可。Dream不依赖Write Hook，由Memory Orchestrator按`schedule` / `manual`创建长期Memory maintenance job。Digest/Dream模型都不常驻监听对话，不自行决定何时读写，只返回严格proposal，再由gateway校验并进入单写者。
- [ ] **任务归属、执行资源与单一事实来源**：Digest/Dream始终归属于Memory所属owner Agent A，proposal也只能写A的active Provider；Data → Memory为两类任务分别保存可选`executorAgentId`（`null`=A自己）、`modelMode: inherit | fixed`和task model。executor B只提供其Home Account connection、adapter/runtime与已验证task model，不取得Memory所有权，也不是Memory Hook执行者。`inherit`在入队时解析executor当时的默认聊天模型，`fixed`只可选择executor Home Account同一connection下、已通过对应任务资格验证的模型，允许聊天使用高能力模型而Digest/Dream分别选择低成本模型。两类资格分别验证；隔离任务不得携带B的聊天历史、Memory、system prompt、Workspace、Tools或sessionState。入队冻结`memoryTaskSnapshot={ownerAgentId,executorAgentId,accountId,kind,provider,modelMode,taskModel,verificationId}`、连接/运行时稳定指纹与任务配置版本；snapshot中的`executorAgentId`为解析后的实际执行Agent（配置为`null`时写owner id），且不持久化connection或secret。不可用时明确失败/暂停，无静默模型、Account、Agent或聊天模型fallback；未来fallback若需要必须另立显式契约。
- [ ] **配置迁移一次完成**：把M2现有全局`memory.digestTrigger`、字符阈值/cron等设置按明确默认规则一次迁移为per-Agent Digest配置；Dream从第一版即使用per-Agent配置。迁移完成后全局旧键不再被runtime读取或写入，不保留兼容别名、双读优先级或两个UI入口；回滚只按迁移备份/版本执行。
...
- [ ] dream 后统一重建派生索引并批量发布新的常驻索引版本；批量大小、schedule、超时和重试均引用配置，任务模型使用上述Account内可验证选择，不把供应商或“便宜模型”具体名称写死。手动“立即Dream”与自动schedule走同一持久job、幂等与状态机，重复点击不创建并行维护写者。
- [ ] **Data → Memory真实页面在后端之后接入**：Agent设置新增Data → Memory，展示唯一主要Memory Provider（默认`vera.markdown`；仅列实际安装且声明`memory-provider`能力的Provider）、条件化Provider配置/路径/连接状态、待整理上下文大小、Digest/Dream各自的执行Agent、任务模型与运行状态、Dream schedule/上次/下次执行、“立即Dream”及长期记忆管理；待整理上下文是尚未Digest的已保存Message水位，不是模型context窗口或Digest后删除的缓存。页面可引用Recall/Write Hook的启用状态但不保存第二份开关；Digest/Dream各自的`executorAgentId`、任务模型、trigger、schedule等Memory领域配置只存Data一侧，不写进Hook binding。普通第三方MCP不进入Provider列表。M4页面先完整支持已实现的`vera.markdown` Provider，不做不可用的“自定义”假选项；通用自定义Provider安装与runtime依赖Phase 6。

**M4 验收**：契约先于代码落地，后端Dream与任务模型配置闭环后才接Data → Memory页面；Recall/Write Hook、MCP主动访问、Digest和Dream路径的触发者/输入/权限测试证明模型不能自行监听或越权写入。关闭Recall只停止自动注入，关闭Write只停止自动Digest，均不影响Message保存或其余显式路径；Dream不依赖Hook。Digest与Dream始终写owner Agent的Provider；选择executor B时只借用B的Home Account connection/runtime与已验证task model，输入中不含B的聊天历史、Memory、system prompt、Workspace、Tools或sessionState。`inherit/fixed`解析和`memoryTaskSnapshot`可复现，未验证或失效executor/model明确失败且零写入、无静默fallback。Hook开关与Data配置各自只有一个事实来源，现有全局Digest配置一次迁移后runtime不再读取旧键。重复dream和重复手动触发幂等；merge后source/双链不丢；错误推断归档可恢复；权重可由输入完全复算；索引批量换版不会改变正在进行的session；dream失败有明确状态且不阻塞主流程。Data → Memory只列真实可用Provider，默认`vera.markdown`的路径/待整理量/任务状态/schedule/立即Dream/长期记忆管理均读写真实API，刷新后回显且无重复配置事实来源；自定义Provider未实现时不得出现可选假状态。
...
- [ ] **5.5.7 `scripts/agent-daemon.js`**：新进程，启动读Tailscale私网gateway URL / agent token，并按Account报告唯一Workspace、CLI binary/runtimeCapabilities；收到已获租约的`run.requested`后只执行与回传，不创建Run或自行切Account。迁移M2/M4前必须先在`adapter-interface.md`冻结并实现专用digest/dream request/result内部通道、取消/超时、安全摘要、`memoryTaskSnapshot={ownerAgentId,executorAgentId,accountId,kind,provider,modelMode,taskModel,verificationId}`与无fallback路由；gateway从Data → Memory配置解析executor及其Home Account/已验证任务模型，daemon不得自行选择Account、Agent或fallback，wire不携带executor的聊天历史、Memory、system prompt、Workspace、Tools、sessionState、connection或secret，proposal只能写owner Agent Provider。不得复用聊天Run/Message/Activity/sessionState，迁移验收前不退役进程内任务adapter。心跳缺失3次后exit(0)，不得私网失败后fallback到公网域名。
...
- [ ] **6.4 联邦能力与配置补全**：Agent组合设置补齐与基本信息/状态/来源并列的Skills、MCP、Hooks、Agent Plugins独立目录；Hooks明确是通用事件自动化能力目录，不被Memory两项穷举，默认内置`vera.memory.recall`与`vera.memory.write`。Recall在消息编译前自动检索+注入，Write在completed Message保存后更新待整理水位并按Data配置请求Digest；二者均为gateway程序Hook，不显示执行Agent或任务模型。其他通用MCP/Hook unit可按自身契约显示启用开关与执行Agent，已选执行Agent不可用时显示`!`；Digest/Dream不是Hook unit，其可选`executorAgentId`、task model、trigger与schedule只在Data → Memory保存。Skill管理、Tools policy与`runtimeCapabilities`消费者逐项闭环；Appearance的产品配置与Web闭环归F4，本项只验证三端读取同一gateway保存值，并处理不可编辑的safe-area/输入法平台叠加，不再重做主题系统。
