...
- [x] **4.2 Speaker view 编译层**：新模块 `src/spaces/view-compiler.js`，输入 `(store, space, agentId, account, triggerMessage)` 输出 prompt 文本；`run-controller.runAsync` 调它替换手拼 promptText，`messages.js` 触发 fan-out 不变。按 ground truth 2.3（2026-07-04 补三条）实现：
  - **只 inject message，不 inject activity**——思考链/工具链不进任何 agent 的下次 prompt（包括本人）。此处历史实现曾把工具历史交给adapter opaque state；现行C1契约改为API必要tool call/result由gateway AgentSession history持有，CLI由generation provider binding持有。这是"发言 ≠ 过程"的边界。Phase 5 的 `fetch_detail`/`fetch_more` 主动调阅是这边的逃生口（按需、带预算），Phase 4 不实现但接口留位。
  - **群聊视角以声告段注入，不伪装成该Agent自己说过的话**：Phase 4实现把他人气泡聚合为署名声告段塞进`ctx.prompt.text`。现行C1在此基础上规定API wire只收bounded messages；每个main Run的稳定history仅把当前trigger Message的最小署名信封与本Agent回复成对保存，累计声告/Recall/常驻块不进稳定turn，避免其他Agent触发时出现孤立assistant轮次。
  - **编译层无状态**：每次 run 临时查 `messages.json` 派生 delta，不维护"已投递水位"；幂等。注入段配置上限（最近 N 条/总字数上限，超了提示"更早的见 fetch_detail"）放 `src/core/config.js`，不硬编码。历史实现的常驻索引按`(account,Space)`首次注入；现行C1改为每个`agentSessionId + generation`首Run注入，索引是稳定前缀、群状态是volatile tail。
...
| Memory注入预算 | 全局 `/api/settings` | `#/settings/system` | resident index / retrieval injection | F4控件与resident index热更新已闭环；继续作为gateway全局预算 |
| Memory Provider / Digest / Dream | per-Agent `/api/agents/:agentId/memory/_config|_options|_status` | `#/settings/accounts/:agentId/data/memory` | Provider facade / Digest / Dream | M4后端与旧全局Digest一次迁移已完成；Data → Memory真实页面待接入 |
...
| Agent Plugin | Extension Package内per-Agent扩展类型 `[Phase 6]` | 全局Extension管理；不增加第五个Agent设置目录 | agent daemon | 分类保留，绑定/状态入口待Phase 6契约；不得混入Skills |
| Space Module | per-Space `[Phase 6]` | `#/settings/extensions` + 当前Space设置 | 沙箱Module host | 分类已定，manifest/API待Phase 6契约 |

**完成标准**：手机蜂窝网络下，主页无底部标签且当前Space聊天占满主区；右滑双栏可按Agent/群切换、新增、重命名、归档和恢复Space，打开期间切换Space不收起，历史与供应商会话连续性不丢；左上目录开关、中间当前Space设置与右上全局Settings职责不串且设置页替换聊天主区；≥2 agents 的 Space 完成一次广播 + 一次定向，被 @ 的 agent 回复prompt含他人署名发言且无误用assistant角色；4.6范围内配置项前端可达且consumer真实生效。`scripts/verify.mjs` 加对应端到端。
...
> 提前量（2026-07-03，与 Theta 确认）：**最小闭环提前落地**——vault骨架+文件格式+常驻索引会话首消息注入+手动保存入口（API）。**2026-07-13边界修订**：旧“agent文件工具直读直写”已废止，M1起所有程序写入统一提交gateway单写者。**2026-07-13 MCP修订**：默认Vera Memory Provider（`vera.markdown`）的Agent runtime读写/检索统一走gateway第一方per-Agent Vera Memory MCP，owner前端HTTP仅为管理入口。**2026-07-15 Provider修订**：Obsidian兼容是`vera.markdown`特性，不是未来所有Memory的强制存储形态；M1–M3已完成实现仍只以该Markdown vault为真相，自定义Provider只冻结契约语义，通用安装/运行时留Phase 6，不伪称已经实现。第一方Vera Memory MCP长期作为所选Provider的统一Agent访问facade，当前尚未接通自定义Provider。检索注入、派生权重、dream仍按本阶段推进。
>
> **默认 Vera Provider 的 vault位置策略**（2026-07-04，与 Theta 确认 + 联邦形态对齐）：vault 热数据**只在 VPS**（`/home/theta/.vera/memory/`，联邦后 gateway 跑在 VPS），所有agent daemon通过gateway第一方Vera Memory MCP远程读写，本地不再有“原版”——避免双写冲突/双读漂移，保持single source of truth。**备份走git镜像**：把VPS vault init成git repo，每次整理后`git push`到私有GitHub repo；Mac上`git pull`只读备份并查看版本历史。派生的`<vault>/.vera-index/`必须加入vault Git忽略，备份/迁移不复制，恢复后由权威Markdown重建。rsync冷备份作次要手段，不替代git镜像的版本维度。Phase 3-4期间vault仍在本机Mac，Phase 5.5联邦落地时随数据迁到VPS并接通带agent token的MCP transport。未来自定义Provider可没有本地路径或Markdown文档，其自身位置/连接状态按Provider配置展示，不套用本段vault迁移与git备份规则。

**执行规则**：已完成历史链为 D0 → M1 → M1.5（Memory MCP facade）→ M2 → M3 → M4后端；当前剩余依赖顺序固定为 **C1（SpaceSession/AgentSession）→ M3.1（真实embedding与merged slug修复）→ M4页面及Dream语义收窄 → F1 → X1**。每个切片的细化契约是该切片第一项，先改契约、再实现、再以临时数据目录验收并独立提交。C1先建立可归档窗口、per-Agent generation与API规范history，M3.1才能把Recall sidecar可靠绑定到generation；Data页面的“当前窗口压力”也必须读取C1真实数据，不用估算假控件。X1必须等待Memory/Data与F1都完成后统一收口。
...
- [x] 冻结Agent/Account/Execution/Workspace关系：每个Agent恰有一个Home Account与独立Memory；取消旧 `Agent 1:N Account` 所有权；每个Execution固定绑定一个Account，主Execution保留Home Account，subagent可在 `authorizedAgentIds` 授权后使用其他Account；每个Account同一时刻只允许一个活跃Execution租约。Workspace、连接/runtime与CLI执行边界按 `accountId` 隔离，Account 1:1 Workspace；SpaceSession/AgentSession与API规范history由Vera持有，CLI thread/resume id只作为generation的provider binding。实际文件在daemon宿主，gateway只保存绑定、策略、状态与校验信息。此项本轮只冻结文档，现有Phase 4代码/UI仍是待迁移旧形态。
...
- [x] **M2契约先行（历史完成形态）**：`scheduled/realtime`为互斥自动策略、manual始终可用；realtime统一按已保存completed Message的Unicode字符水位，默认16000；Run结束不冒充session-end dream。2026-07-15新契约已把Dream固定为Memory Orchestrator按schedule/manual创建的聊天外维护job，不再保留session-end兜底，也不把Dream登记为Hook unit。事实地址/值由结构槽规范化后派生且不进frontmatter；同事实保留原slug并合并SourceRefs，明确纠错才允许同slug supersede，模糊匹配跳过。
...
- [x] 已接通M2当前全局`memory.digestTrigger`的`scheduled`/`realtime`/`manual` consumer与五段cron；实时阈值只用Unicode字符水位，M2不实现session-end Dream。M4按上文一次迁移为per-Agent配置后停止读取这些全局旧键；无关Settings更新不越过cron触发catch-up，成功watermark使用持久toSeq不因后续可见性变化倒退。
...
- [x] 在`test/adapters/`按同一行为矩阵回归Ollama/OpenCode：固定kind/provider、stream/session、稳定/临时prompt分层、容量裁剪、错误/取消/timeout、secret、cleanup和digest隔离断言，不抽取运行时BaseAdapter；`verify.mjs`的临时gateway+stub Ollama黑盒已通过，真实Ollama 0.23.2/Gemma已完成chat与structured digest transport smoke，但这不等于精确模型的raw语义资格通过。
...
- [x] **M2已落执行基线（2026-07-15契约修订后保留为历史事实）**：当前实现由gateway绑定owner Agent并使用已解析Account/model执行Digest，job入队冻结执行快照，Memory仍归owner Agent；尚未实现Data → Memory任务模型选择器、自定义Memory Provider或真实daemon/MCP transport，不得把下面M4/Phase 6新契约倒写成M2已完成能力。产品层把M2的completed Message水位/trigger编排归入`vera.memory.write`，M3的自动检索+注入归入`vera.memory.recall`；两者都是gateway程序Hook。Digest/Dream是Memory Orchestrator创建的隔离模型任务，不是Hook unit。MCP目录中的第一方Vera Memory MCP当前由默认`vera.markdown` Provider支撑，未来经统一Provider契约路由所选Provider。普通第三方MCP不自动成为Memory Provider。
- [ ] slug/钩子行、source、双链、stain裸hex、同事实targetFactId、手动Memory adopt与纠错supersede校验已落地；fact catalog已把现有`type`交给executor并回归mapped/unmapped旧分类可见，executor可见chunks也已移除内部chunk id以保证证据只能引用`messageId`。固定raw夹具现已永久接入真实gateway路径，覆盖无复用价值、无来源推断、agent自创偏好、跨job同事实与明确纠错；`gemma4:e4b`当前在跨job同事实复用处把相同值错误提议为`supersede`，权威validator以`invalid_proposal`拒绝且vault不变，因此该项仍未通过。
...
- [x] **Codex真实模型闸门**：本机真实`kind=cli, provider=codex, model=gpt-5.6-sol`Account已显式跑通chat和当前M2端到端digest；真实使用`--output-schema`、digest不resume聊天thread且JSONL无tool item，proposal再通过gateway权威validator。
- [ ] **Gemma真实模型闸门（延后至Phase 5.5网关部署后）**：原生Ollama adapter已以`kind=api, provider=ollama, model=gemma4:e4b`直连完成chat、structured-output和隔离digest transport smoke；固定raw语义夹具仍在同事实复用处失败，所以该精确model/tag尚未验证，不得作为Phase 5.5“已验证”候选。按owner 2026-07-14决定，此项保留失败证据并延后到VPS gateway与daemon真实部署完成后，在同一精确runtime/model/tag上重跑；延后只解除M3的执行阻塞，不等于验收通过。失败保持安全`invalid_proposal`和vault零变化，不用prompt特判、adapter猜测修复或放宽validator换取绿灯。

**M2当前验收（2026-07-14，工程链路收口，Gemma raw语义资格延后）**：普通`npm test`为173通过、2个真实smoke默认skip，普通`verify.mjs`为73/73通过；显式`VERA_TEST_OLLAMA_NATIVE=1`真实闸门为73通过、1失败，唯一失败即Gemma raw语义资格。Codex adapter以本机`codex-cli 0.144.2`、`gpt-5.6-sol`完成真实gateway chat与当前M2 digest，聊天持久化threadId，digest在空临时cwd使用ephemeral/read-only/never、强制`--output-schema`且无tool item，proposal经Vera完整validator后写入带SourceRef的Memory；真实capability probe另验证JSONL事件翻译与mid-flight abort。原生Ollama adapter直连Ollama 0.23.2 `/api/chat`，`gemma4:e4b`的chat与structured digest transport可用，但固定raw gateway夹具发现它会把相同事实再次确认误判为`supersede`；gateway正确拒绝且未污染vault，因此不能把该模型登记为已验证digest executor。真实smoke的chat安全覆盖为read-only，生产默认仍为workspace-write。OpenCode聊天与digest代码未删除，但OpenCode digest已退出生产dispatch/M2闸门并明确`executor_unavailable`。按owner决定，未通过的Gemma raw语义资格作为Phase 5.5网关部署后的显式遗留验收项，当前不登记资格、不屏蔽失败，但不再阻塞M3开工。

### P5-M3 — 三渠道检索、注入预算、横向扩展与正文展开

- [x] **M3契约先行（历史完成形态）**：已补齐retrieve/fetch_more/fetch_detail、分支游标、token预算、program-owned去重sidecar、使用统计、置顶、安全错误与“M3不新增SSE”边界；当时冻结`m3-r1`的BM25 + char-trigram TF-IDF等宽召回、2-hop图扩散、五项精确权重、交汇函数、两阶段去重、两级投影、可借用软配额、稳定tie-break、`vera-utf8-v1`预算和服务端snapshot游标。现行契约已由C1/M3.1修订：sidecar归`agentSessionId + generation`，query语义召回改用真实embedding，char-trigram仅保留给确定性近重复聚类；旧通用`sessionState/recompileForNewSession`由generation/provider binding取代。
- [x] 常驻索引按 R3 批量换版，只在新的外部provider binding建立稳定前缀；用户置顶 + 按派生权重选出的top条目合计受 `memory.injectionBudgetResidentLines` 限制，普通编辑不逐条打穿 prompt cache。
...
- [x] **M4契约先行（2026-07-15产品语义已确认并冻结）**：`vera.memory.recall`在消息上下文编译前执行自动检索+预算内注入；`vera.memory.write`在completed Message保存后观察待整理水位，并按Data → Memory的`realtime` / `scheduled`自动策略请求Memory Orchestrator创建Message范围Digest job，owner的`manual`请求绕过自动Hook开关。两项Hook均由gateway程序执行，不选择执行Agent或任务模型；关闭Recall只停止自动注入，MCP主动读仍可；关闭Write只停止自动Digest，Message仍保存，`pendingContext`仍可由已保存Message与每个`(agentId,spaceSessionId)` Digest watermark计算，owner手动Digest与MCP手动保存仍可。Dream不依赖Write Hook，由Memory Orchestrator按`schedule` / `manual`创建长期Memory maintenance job。Digest/Dream模型都不常驻监听对话，不自行决定何时读写，只返回严格proposal，再由gateway校验并进入单写者。Dream只维护既有长期Memory的重复、结构、描述、双链与有明确替代项的冗余归档；它不接收原始Message，因此不得纠正事实值或凭常识宣布内容过时。Provider能力分别声明`digest.ingest`与`dream.maintenance`操作，不假设不同Memory结构共享同一处理器。
...
- [ ] **5.5.0 Home Account / Execution / Workspace迁移**（其余联邦运行项的前置）：先完成P5-C1，再按更新后的契约把现有 `Agent 1:N Account` 数据和Account组合UI一次迁移为每Agent一个Home Account；建立每Execution固定 `agentId + accountId`、主Execution只用Home Account、subagent按 `authorizedAgentIds` 使用其他Account的授权关系。同步建立通用MCP/Hook unit执行Agent可用性判断与安全候选摘要；已选执行Agent不可用时保留选择并显示`!`、不得自动改投。`vera.memory.recall`与`vera.memory.write`是gateway程序Hook，不需要执行Agent候选；Data → Memory的Digest/Dream可各自选择executor Agent，并只能从该executor的Home Account同一connection中取得已通过对应任务资格的安全model候选摘要，不得返回secret或跨连接自由文本模型。任务Memory归owner Agent，executor只提供执行连接/runtime。Account增加唯一活跃Execution租约，竞争时排队或返回 `account_busy`，结束/取消/超时/失联必须释放。每Account建立唯一Workspace绑定，gateway只保存daemon宿主、绑定、策略、状态与校验时间；Workspace、连接/runtime与CLI执行边界按`accountId`隔离，SpaceSession/AgentSession及API规范history仍由gateway持有。迁移前后的旧字段与UI不得并存为兼容双名。
...
- [ ] **5.5.5 `/api/agent/*` 路由层**：`src/api/agent-routes.js`新建——Bearer token识别Agent；login只登记daemon、per-Account Workspace/runtimeCapabilities、候选授权Account及CLI provider bindings，不选择Account或取得租约。gateway创建主Run或父Run调用`POST /api/agent/runs/:id/subagents`时生成pending Execution；调度器取得目标Account唯一租约后转running、广播`run.started`并发分型`run.requested`。daemon不得重复创建/认领Run；终态/登出/超时释放租约。CLI binding更新必须绑定租约持有者、`agentSessionId + generation`与opaque version；API daemon不得同步完整history，只能用`api-result`提交本Run的reply Message ids与安全tool/usage，由gateway构造最小trigger+assistant turn并做historyVersion CAS。
...
- [ ] **5.5.7 `scripts/agent-daemon.js`**：新进程，启动读Tailscale私网gateway URL / agent token，并按Account报告唯一Workspace、CLI binary/runtimeCapabilities。`run.requested`按input.kind严格分型：main冻结`spaceSessionId + agentSessionId + generation`，isolated subagent的session字段为null且终态销毁；API main只消费bounded messages/historyVersion并在completed前走`api-result` CAS，CLI main消费promptText/provider binding。daemon另处理专用compact request/result，不把压缩输出发成聊天。迁移M2/M4前必须先在`adapter-interface.md`冻结并实现专用digest/dream request/result内部通道、取消/超时、安全摘要、`memoryTaskSnapshot={ownerAgentId,executorAgentId,accountId,kind,provider,modelMode,taskModel,verificationId}`与无fallback路由；gateway从Data → Memory配置解析executor及其Home Account/已验证任务模型，daemon不得自行选择Account、Agent或fallback，wire不携带executor的AgentSession、checkpoint、API history、CLI provider binding、Memory、system prompt、Workspace、Tools、connection或secret，proposal只能写owner Agent Provider。不得复用聊天Run/Message/Activity或会话状态通道，迁移验收前不退役进程内任务adapter。心跳缺失3次后exit(0)，不得私网失败后fallback到公网域名。
- [ ] **5.5.8 mock daemon + verify.mjs 拆分**：CLI mock走login（带bindings）→ 分型run.requested → delta/activity/message → provider-binding CAS → completed；API mock走bounded messages/historyVersion → reply Message定稿 → api-result CAS → completed，并证明完整history只在gateway；另覆盖native/checkpoint compact request/result、history conflict、isolated subagent零session持久化与logout。gateway内部一致性测试保留旧mock adapter路径。
...
4. 重启VPS gateway → Mac daemon经Tailscale自动重连并取回CLI provider bindings；API AgentSession由gateway规范history继续；手机Tailscale身份仍有效且SSE按since恢复
...
- [ ] **6.4 联邦能力与配置补全**：Agent组合设置固定补齐Skills、Hooks、MCP、Data四个平级目录；不增加第五个Agent Plugin入口，也不把Agent Plugin混入Skills。Hooks明确是通用事件自动化能力目录，不被Memory两项穷举，默认内置`vera.memory.recall`与`vera.memory.write`；Recall在消息编译前自动检索+注入，Write在completed Message保存后更新待整理水位并按Data配置请求Digest，二者均为gateway程序Hook，不显示执行Agent或任务模型。MCP默认内置`vera.memory`，仅展示启用状态、可用性与工具清单，不出现semantic Agent/model；普通第三方MCP/Hook按自身契约显示字段。Digest/Dream不是Hook unit，其executor、task model、trigger/schedule与Provider能力只在Data → Memory保存；Data不投影Hook开关。Agent Plugin仍是Extension Package可包含、由daemon承载的扩展类型，其统一管理入口待6.5 Extension契约落地。Skill管理、Tools policy与`runtimeCapabilities`消费者逐项闭环；Appearance的产品配置与Web闭环归F4，本项只验证三端读取同一gateway保存值，并处理不可编辑的safe-area/输入法平台叠加，不再重做主题系统。
- [ ] **6.5 Extension体系**：先完成Extension Package manifest/安装/版本/权限契约，再实现Skill/MCP/Hook各自runtime、daemon侧Agent Plugin与隔离Space Module；同时冻结并实现可由Extension Package声明的`memory-provider`能力契约、安装状态与Provider配置/健康检查，使自定义Provider可以把自身文件、数据库或远程服务作为事实来源而不强制转成Obsidian Markdown。普通第三方MCP即使提供memory命名工具也不得自动登记为Provider；只有声明并通过Memory Provider契约验证的已安装unit才进入Data → Memory列表。Agent各能力目录可发起导入/加载，但底层统一写同一Extension Package安装事实来源，安装后只绑定对应unit，不建第二套包格式或万能Plugin runtime。Settings继续负责全局安装/卸载，Account/Space分别启用。Space Module使用可销毁sandbox并提供Web/Android/iOS一致bridge，未启用时零加载、崩溃不影响聊天Shell；第三方代码不得直接进入主DOM或持有gateway/secrets/宿主文件权限。
