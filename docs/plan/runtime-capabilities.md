...
- [ ] 在可用的真实浏览器控制环境完成手机与桌面验收：头像/联系人入口、
  默认与指定Agent定位、前后翻页、四个平级目录、返回关系、deep-link、
  loading/empty/error、长名称、禁用动作，以及Hooks/MCP真实binding开关。

阶段A–B实现与自动化验证已完成；这里只保留未完成的浏览器闸门。
