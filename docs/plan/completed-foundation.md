...
- 本切片最新验收：`npm test`为352通过、3个显式opt-in跳过，`npm run build:web`通过，`node scripts/verify.mjs`为95/95；固定`PORT=3210`临时gateway以显式loopback development豁免启动成功，`GET /api/health`与`GET /api/bootstrap`均返回200。
