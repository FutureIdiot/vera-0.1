...
- 本切片最新验收：`npm test`为383通过、3个显式opt-in跳过，`npm run build:web`通过，`node scripts/verify.mjs`为95/95；黑盒Codex/Ollama路径均通过真实daemon HTTP/SSE而非gateway-local执行。固定`PORT=3210`临时gateway启动成功，`GET /api/health`返回200。
