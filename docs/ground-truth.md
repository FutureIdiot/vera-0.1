...
- **AgentSession是某个Agent在某个SpaceSession中的主聊天模型上下文**。一个SpaceSession可有多个AgentSession，唯一键为`(spaceSessionId, agentId)`；私聊通常只有一个，群聊为每个实际参与响应的Agent分别持有。群成员共享SpaceSession边界，但绝不共享供应商history、CLI thread或API模型上下文。subagent Execution仍是Run级隔离任务，不自动成为Space成员或污染主AgentSession：其Run的`agentSessionId/contextGeneration`为`null`，只使用本Run冻结的isolated execution context，结束后不保存API history或CLI provider binding。
- AgentSession由Vera持有逻辑真值，至少包括`generation`、上下文检查点、最近完整轮次、容量估算与provider binding元数据。Account只提供本次主Execution使用的连接/runtime；CLI的thread/resume id只是某一AgentSession generation的外部绑定，API Agent的规范history/checkpoint由Vera保存并提供，不能把opaque provider state当成API会话真值。
...
- 裸`/compact`是当前SpaceSession的手动控制命令：私聊作用于唯一Agent，群聊默认作用于当前全部Agent；各Agent独立成功或失败，不要求跨provider原子回滚。命令本身不保存为Message、不触发普通Run、不进入Digest。压缩期间仅目标Agent的新Run排队。
- 裸`/new`是SpaceSession级控制命令：必须一次作用于当前Space的全部Agent，当前有未结束Run或压缩时返回`session_busy`而不暗中取消。成功时归档当前SpaceSession及其全部AgentSession/provider bindings，创建新的active SpaceSession和generation 1 AgentSessions；旧窗口只读可查。新窗口继承Space成员/规则、Agent身份与长期Memory能力，不继承旧checkpoint、最近轮次、Recall交付/cursor、CLI thread或API history。`/new`本身不自动Digest、Dream或compact；尚未Digest的旧Message仍可按明确的单一SpaceSession范围整理。
...
| 能力与数据设置 `[P5前端目录与内置binding；Phase 5.5/6通用runtime]` | Agent设置固定为Skills / Hooks / MCP / Data四个平级目录；Data首先承载Memory。四类都是独立资源，不嵌进Agent身份字段 |
...
| 模型名 | 当前使用的底层模型 |
| 连接与项目上下文 | provider连接、Workspace与runtime随Account；SpaceSession/AgentSession及API规范history属于Vera，CLI外部会话仅以provider binding挂到AgentSession generation |
...
- **每个Account同一时刻只允许一个活跃Execution租约**，无论执行者是Home Agent还是获授权的其他Agent；竞争请求必须排队或明确返回 `account_busy`，不得让多个Agent/daemon并发驾驶同一CLI provider binding与Workspace
- Memory始终按 `agentId` 隔离并跟随该Agent的所有Execution；Workspace与连接/runtime按`accountId`隔离；AgentSession按`spaceSessionId + agentId`建模并由Vera持有。Seat不携带`accountId`，具体账户只由Execution绑定
...
- **发言与过程的边界**（2026-07-04补，2026-07-15按AgentSession修订）：Message（气泡）是对外发言，是 agent 在 Space 内唯一能被其他成员看见的输出，经编译层署名注入他人的下次 prompt；Activity（思考链/工具链）只服务于同期观察的用户，**不进任何 agent 的下次 prompt**——包括 agent 本人。API Agent的必要tool call/result由gateway写入其AgentSession规范history；CLI工具历史由该generation的外部provider binding持有，gateway不把Activity二次注入。其他 agent 想要细节只能靠 Phase 5 的 `fetch_detail`/`fetch_more` 主动调阅，按需、带预算，不是默认注入。这是"时间线对用户全展开、prompt 层只看气泡"的产品语义边界。
- **群聊视角的注入形态**（2026-07-04补，2026-07-15按API history修订）：其他成员的气泡以"群内最近发言"这一明确声告段进入本轮volatile输入，不伪装成该Agent自己说过的话。编译层在该Agent上次发言之后到当前触发之间派生这段delta，无独立投递水位。CLI与API共享同一语义编译结果，但wire分型：CLI只收`promptText + providerBinding?`，API只收`messages + historyVersion`。API规范history按每个main Run原子保存一对turn：输入侧只保留当前trigger Message的带来源署名信封，输出侧保留本Agent回复及provider确需的安全tool transcript；累计群聊声告、常驻Memory块和Recall投影不写入稳定turn。这样由其他Agent发言触发的回复也有明确前因，不产生孤立assistant轮次。CLI provider thread可能按供应商能力保留已投递文本，但gateway不把它冒充API规范history；compact后两者都只继承checkpoint与最近完整轮次。
...
| 会话归属 | Vera在gateway持有SpaceSession、AgentSession及API规范history/checkpoint；CLI thread/resume id由daemon执行、gateway按`agentSessionId + generation`保存provider binding，不再存在通用`sessionState`或`sync-state` |
...
**记忆整理流程：**
- Digest把尚未整理、已完整保存且对该Agent可见的Message转成长期Memory，并在成功后推进对应`(agentId, spaceSessionId)`的整理水位；它有原始Message证据，负责create/update/supersede/archive/skip。Dream只维护active Provider中已经存在的长期Memory，负责明确重复项合并、结构/描述/双链整理和冗余归档；它没有原始Message正文时不得纠正事实、改变事实值或把猜测升级为事实。两者都不是普通聊天Run，Digest也不等于Dream
...
- 新Agent默认安装并启用`vera.memory`、`vera.memory.recall`与`vera.memory.write`。Digest/Dream分别在Data → Memory保存`executorAgentId`与任务模型；默认`executorAgentId=null`表示Memory所属Agent自身，任务模型默认`inherit`该执行Agent Home Account的聊天模型。owner可为两类任务分别选择另一个可用Agent及其Home Account同一连接下已验证的低成本fixed模型。Dream自动调度默认`manual`以避免未确认的后台成本，owner可改为每天、每周或自定义schedule
- 流程环节：分块（gateway程序）→ 隔离proposal（选定Account adapter）→ 校验与写入（gateway单写者）
- Digest与Dream job始终属于Memory所属Agent（owner Agent），proposal最终也只能写入owner Agent的active Provider。每类任务可在Data → Memory独立选择执行Agent；`executorAgentId=null`解析为owner Agent，选择B则只借用B的Home Account连接/runtime执行隔离任务，不把Memory归属或写权限转给B。`modelMode=inherit`使用入队时执行Agent Home Account的聊天默认模型，`modelMode=fixed`允许选择同一连接下真实可用、且已通过对应任务资格验证的模型。Digest资格与Dream资格分开验证；通过一个任务不自动取得另一个任务资格，前端也不接受任意模型自由文本
- 入队时gateway冻结`ownerAgentId + executorAgentId + executor Home Account id`、provider、连接/运行时稳定指纹、实际taskModel、对应任务verification与owner active Memory Provider binding/config version快照；snapshot不持久化connection或secret，retry复用原快照，设置变化只影响新job。送入模型的隔离任务包只包含本次任务明确允许的Message/Memory安全投影与任务指令，不继承执行Agent B的AgentSession、checkpoint、API history、CLI provider binding、Memory、普通system prompt、Workspace或Tools，也不包含Account connection/secret
- 所选执行Agent、其Home Account、任务模型或资格不可用时任务明确进入`failed`终态并显示安全原因，配置保持原值；0.0.1不引入`paused`状态，也不得静默改投owner Agent、其他Agent、其他Account、聊天默认模型、其他模型或其他Provider。“任务模型”是预先选择的成本/能力策略，不是失败fallback
...
- 每个Agent的Data → Memory中，`digest.trigger.mode`只选择一种自动策略：`scheduled`与`realtime`互斥，`manual`表示关闭自动整理；无论选择哪种策略，owner与可信Agent都始终可以手动提交整理。`realtime`不是每轮都跑模型，而是按`(agentId, spaceSessionId)`统计尚未整理、已完整保存的Message正文Unicode字符数，达到配置阈值后异步排队；Digest范围不得跨SpaceSession。Activity、流式delta、工具过程与provider token估算都不进入水位。聊天Run不等待整理。
- M2 的 Message 整理与 M4 的 Dream 是两种 job：Run 结束不等于外部 session 结束，M2 不把每次 Run 结束伪装成session-end Dream，也不增加失败后偷偷换模型的兜底开关
- **默认`vera.markdown` Provider存储形态**（2026-07-02定稿）：文件即真相——每条记忆一个markdown文件、语义化slug、`[[双链]]`互联，记忆库即Obsidian vault；数据库仅为派生索引。以下vault、slug、frontmatter、Obsidian外部编辑与gateway单写者细则只约束该默认Provider，不强迫自定义Provider转换为这种物理格式。详见memory-hook.md《修订：文件库架构》
...
- 对`vera.markdown`的所有程序写入（主Agent、subagent、CLI adapter、Hook与Dream）只能向gateway提交proposal/operation，由Memory单写者校验并原子落盘；不得直接写vault。用户通过Obsidian所做的外部编辑由gateway重扫、校验并刷新派生索引，不构成第二个程序写入通道。自定义Provider必须在自己的契约边界内提供等价的串行化、版本冲突与原子提交保证，gateway不绕过Provider直碰其原生存储
- **Vera Memory MCP是gateway托管的第一方per-Agent逻辑服务**：Agent runtime的读取、写入提议、检索、横向扩展和正文展开只走Vera Memory MCP tools，由Memory facade路由到active Provider。对`vera.markdown`不得使用`fs.read/fs.write`直接碰vault；对自定义Provider也不得绕过其driver。MCP工具参数不接受`agentId`，gateway从可信Execution/agent token上下文绑定身份；切换Account不切换Memory。owner前端继续使用HTTP管理API，但HTTP与MCP必须调用同一Provider facade，不得复制业务实现
...
- Dream与Digest共享“job固定属于Memory所属Agent、执行Agent与任务模型在Data → Memory中选择、只借用执行Agent Home Account连接/runtime、入队冻结快照、隔离上下文、proposal只回写owner active Provider、失败不静默fallback”的规则，但使用独立资格、独立配置和独立job状态；Dream不能借用Digest验证记录
...
- 每个Account恰有一个Workspace；Workspace绑定、连接与runtime数据以`accountId`隔离。AgentSession不属于Workspace；只有该AgentSession generation的CLI provider binding会引用实际执行Account
...
**记忆整理**
- active Memory Provider及其Provider特有配置；路径只对文件型Provider展示，默认`vera.markdown`显示全局vault根与派生Agent子目录
- Digest与Dream各自的执行Agent、任务模型策略与状态；Digest触发时机，Dream的手动/每天/每周/自定义调度与时区。执行Agent只提供其Home Account连接/runtime，任务与Memory归属仍固定为当前owner Agent
...
**全局设置入口**
- Appearance
- Account管理（包含Agent信息、Account连接与进入Agent设置的入口）
...
Agent设置固定为Skills / Hooks / MCP / Data四个平级目录，对应路由固定为`#/settings/accounts/:agentId/skills`、`#/settings/accounts/:agentId/hooks`、`#/settings/accounts/:agentId/mcp`与`#/settings/accounts/:agentId/data`。Skills只管理可加载/卸载的Skill；Hooks只管理Hook unit；MCP只管理MCP unit；Data管理该Agent的数据领域，当前首先提供Memory。Data → Memory只配置Memory结构（技术层为active Provider）、位置/状态、长期占用、待整理内容、Digest、Dream与长期Memory管理，不显示、不投影、不保存Recall/Write binding。MCP和Hook目录中的单元都展示启用开关；是否还需要其他控件由unit契约决定，不能给所有Hook强塞`executorAgentId`。gateway内置的Vera Memory MCP、Recall Hook和Write Hook均不展示执行Agent或模型。Digest/Dream的执行Agent与任务模型是Memory领域配置，只在Data → Memory中按真实执行资格候选展示，不扩张成所有MCP/Hook单元的字段。
...
- **Vera Memory MCP是唯一明确的第一方例外**：因为Memory逻辑身份、active Provider绑定与默认Provider单写队列都在gateway，MCP server/dispatcher也由gateway持有，daemon只做MCP client或把tools映射给CLI/provider。不得把这个例外扩张成gateway代跑任意第三方MCP
...
| Account管理 / Agent设置 | `#/settings/accounts`、`#/settings/accounts/:agentId` | 组合展示Agent身份/状态、Home Account、授权策略及Skills/Hooks/MCP/Data四个平级入口 | 把Agent字段复制进Account；把授权账户展示成该Agent拥有的Account列表；增加第五个Agent Plugin入口；在详情页预取所有子目录数据 |
| Agent Skills / Hooks / MCP | `#/settings/accounts/:agentId/skills`、`.../hooks`、`.../mcp` | 共用单列目录骨架、真实空态和顶栏“添加/管理”；按各自controller注入条目，Hooks/MCP后续接unit binding，Skills后续接Skill契约 | 建万能capability后端资源；在生产代码硬编码示例条目；接口未接通时提供可完成假动作 |
| Agent Data | `#/settings/accounts/:agentId/data` | 列出该Agent的数据领域入口，当前只有Memory；沿用目录列表视觉但不是Skill/MCP/Hook unit资源 | 把Data域伪装成扩展unit；在索引页预取Memory配置、状态或正文 |
| Agent Data → Memory | `#/settings/accounts/:agentId/data/memory` | 以“Memory结构”展示active Provider，查看位置/状态、长期占用与待整理token压力；分别配置Digest和Dream执行Agent、任务模型、触发/调度，提供手动Digest与立即Dream，并进入长期Memory管理 | 显示或复制Recall/Write Hook binding；把跨Space总量说成下一轮精确消耗；展示未安装Provider假选项 |
| Agent Memory管理 | `#/settings/accounts/:agentId/data/memory/library` | 查看、编辑当前Agent active Provider支持的长期Memory；作为Data → Memory的下一级目录按需进入 | 列表页预加载所有Agent的Memory正文；在Provider不支持编辑时伪造CRUD |
...
- Account管理是组合页面，但底层为 `Agent 1:1 Home Account`；其他Account只通过 `authorizedAgentIds` 授权具体subagent Execution使用，不成为该Agent的所有物。Memory与AgentSession归Agent，连接、Workspace与runtime归Account。
...
- Space导航左栏选中Agent/群后，右栏只列出成员集合匹配的活跃Space。新增Space继承当前成员集合并创建首个active SpaceSession；重命名、归档操作只作用于选中的Space记录。Space归档仅写`archivedAt`并保留Space、active SpaceSession、历史与provider bindings；已归档Space从活跃列表移出，在导航的“已归档Spaces”入口查看并恢复，不提供永久删除。`/new`产生的SpaceSession归档另从当前Space的“历史对话”入口只读查看，绝不提供恢复。
- 跨页面只传稳定ID和筛选条件。Agent详情先进入Skills / Hooks / MCP / Data任一平级目录；Data先列出Memory，再进入`#/settings/accounts/:agentId/data/memory`查看配置与状态，最后按需进入`#/settings/accounts/:agentId/data/memory/library`管理长期Memory。Memory两页复用同一Memory领域client，但不得把Provider配置、job状态和正文编辑塞回Account view或同一个巨型页面。
- 持久对象的详情与编辑使用可重载、可前进后退的路由；弹层只用于确认、选择和短暂输入，不用弹层藏完整页面。
- Space目录只属于聊天页，不得带入当前Space设置、全局Settings或其子页。所有设置页都是独立全屏页面，顶栏左上统一为返回、中央只显示一次页面标题，正文不得再重复返回入口或`h1`标题。Skills / Hooks / MCP可在同一Shell顶栏右侧声明“添加/管理”两个页面动作；其他页面没有契约时不得顺手增加。必须复用同一路由和同一份状态，不另造桌面业务实现。
...
- `views/`：Shell、Space导航、Space聊天、当前Space设置、Settings目录、Account列表/详情、Agent capability目录、Agent Data/Memory配置、长期Memory管理、Extension、路径、中控台、系统设置、Appearance分别独立。Skills / Hooks / MCP三个路由可复用一个无HTTP职责的capability目录view；一个view不得同时拥有两个对象的写流程。
