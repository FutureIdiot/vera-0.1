...
| 模型 | `model + modelVersion`；是该Account从所属Agent已接入模型列表中选择的当前聊天模型，不参与owner归属 |
...
| provider/runtime/models | 属于Agent daemon的真实执行能力；Agent报告自己已接入的模型列表，Account只能从该列表选择当前聊天模型 |
| 能力与Data设置 | Skills / Hooks / MCP / Data四个平级目录继续属于Agent |

**Execution（执行）**：一次实际运行的绑定关系。每个Execution创建时固定`agentId + accountId + runtimeRevision + effectiveModel + modelVersion + delegated`；Agent贡献自己的Memory与模型能力，Account贡献本次Space Seat身份、Account Workspace、当前模型选择与相应项目执行权限。Space时间线和附件仍是gateway持有的Space共享数据，不移入Workspace。
...
- `model`是Account级当前选择，不是owner绑定的一部分；`modelVersion`用于并发写入。候选必须来自owner Agent最近登记的`runtimeCapabilities.models`，不允许自由文本、跨Agent借用或静默fallback。首次接入以Agent runtime报告的默认模型初始化；以后模型列表更新若使当前选择失效，保留失效值并阻止新Run，直到User显式重选。
- `effectiveModel`必须等于Run创建时Account选择的实际可展示模型名，并在Execution内冻结；不得为空、写成`default`或回退显示Account名/provider名。换模型只影响后续Run，不改历史Message，也不改变`ownerAgentId`、Memory、Workspace或Agent能力配置。
...
- 换Account Key或Account聊天模型都不改变owner；Agent接入模型列表变化改的是Agent runtime能力，Account只显式选择其中一个。改变Memory Provider placement或Workspace宿主必须走各自显式迁移，不能随登录静默移动数据。
...
| model / modelVersion | Account当前聊天模型选择及CAS版本；与owner Agent词条独立，但值必须来自该Agent已接入models |
...
| Agent runtimeProfile | 版本化、纯JSON且可稳定序列化的便携配置；当前严格为`{schemaVersion:1,kind,provider,model}`，其中`model`是daemon默认模型，不是Account当前选择 |
| Agent runtime snapshot | `hostId/revision/connectionFingerprint/runtimeCapabilities.models`等能力和在线状态由实际Agent daemon派生登记，不属于导出profile |
...
| Account系统管理 | `#/settings/accounts`、`#/settings/accounts/:accountId` | 新建/删除/命名Account，生成/轮换/撤销接入Key，分别查看所属Agent与当前模型，从该Agent已接入模型列表切换模型，查看Workspace、Space与最近20条安全登录审计 | Agent Memory正文、Skills/Hooks/MCP/Data；provider/connection配置或自由文本模型；owner改绑；直接创建空Agent；原始身份头、boot id、Token/fingerprint或宿主路径 |
