...
| 全屏聊天主页 | `#/spaces/:spaceId` | 当前Space时间线、广播、@定向、实时AgentState、输入栏 | Space列表管理、Account编辑、全局设置 |
| Space导航 | `#/spaces`（右滑/抽屉呈现） | 左侧联系人/群头像；右侧所选成员集合的Space列表；切换、新增、重命名、归档/恢复Space；桌面宽屏可钉住为辅助栏 | 当前Space Seat/插件/提醒配置、Account连接配置 |
| 当前Space设置 | `#/spaces/:spaceId/settings` | 当前Space参与Agent、Seat响应规则、消息提醒、Space Module启用与配置 | 切换/新建其他Space、全局扩展安装、Account连接信息 |
| Setting目录 | `#/settings` | 轻量分组入口；不预加载各子页数据 | 把所有设置表单和状态面板渲染在同一页 |
| Appearance | `#/settings/appearance` | 外观实时预览、保存、恢复默认 | 业务行为或页面数据配置 |
| Account管理 | `#/settings/accounts`、`#/settings/accounts/:agentId` | 组合展示Agent身份/状态/Memory与其一个或多个Account连接 | 把Agent字段复制进每条Account记录 |
...
- Account管理是一个组合页面，不改变底层 `Agent 1:N Account`；Memory仍归Agent，连接/模型/外部会话仍归Account。
...
- 顶栏左上按钮只进入当前Space设置；顶栏右上按钮只进入全局Settings。两者不得复用同一个面板。
- 手机右滑打开Space导航，不显示占用聊天边缘的常驻按钮；顶栏中间的当前Space名称+下拉标记是等价可点击入口。桌面触控板横向手势不可靠，仍使用同一标题入口；导航页左下角提供小图钉，宽屏下可把同一导航view钉为常驻辅助栏或恢复为覆盖抽屉，不另造桌面导航实现。手机不显示图钉。
...
- 手机竖屏一次只显示一个主页面；Space导航抽屉覆盖主区而不是压缩主区。当前Space设置、Settings及其子页是独立路由，进入后替换聊天主区，绝不追加在时间线下方或与聊天纵向叠放。桌面也由路由替换主区，只允许已钉住的Space导航作为辅助栏保留；必须复用同一路由和同一份状态，不另造桌面业务实现。
...
- 验收基线：390px中档Android/WebView、iPhone模拟器/真机WKWebView与桌面浏览器都要做Performance trace；缓存后进入聊天可交互目标≤1.5s，模拟4G冷启动目标≤3s，滚动/抽屉动画无连续>50ms long task。预算调整必须先改本文档。
...
- Web由gateway同源伺服；Android/iOS打包本地静态资源并通过用户配置的HTTPS gateway URL连接。原生壳不得内置固定IP、token或secret。
