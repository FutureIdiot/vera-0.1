...
- Space内的数据按层隔离（见第三节）
- Account可以加入多个Space，跨Space保持同一对外身份和项目数据边界；实际代上线Agent可变化。
...
- 换Account Key只改变谁可代表该Account；换provider/model改的是Agent runtime；两者都不迁移Agent Memory
...
- 显式代上线接管先撤销旧Account登录会话并终态化其在飞Execution，再建立新`activeAgentId`；不得让两个Agent短暂并发代表同一Account
...
- **Account访问权**：Account access key由User在Account页生成/轮换/撤销，回答“该Agent是否可代表这个Account”。明文只显示一次；daemon登录必须同时提交agent token与Account key。共享Account key不会共享agent token，也不会让两个Agent读取同一Memory。
...
- **双凭证授权闸门**（Phase 5.5）：登录先以agent token固定`agentId`，再以Account access key固定`accountId`；二者都通过后才可建立该pair的Account会话。每条Execution还必须匹配当前Account会话并取得唯一租约。不存在`authorizedAgentIds`旁路，也不得只凭共享Account key推断Agent身份。
...
- 流程环节：分块（gateway程序）→ 隔离proposal（选定Agent runtime adapter）→ 校验与写入（gateway单写者）
...
- 当前Phase 5实现仍从旧Account字段选择Ollama/Codex adapter；Phase 5.5迁移后改为执行Agent runtime profile。Codex/Ollama的隔离structured-output、无Tools、无fallback与gateway完整validator规则不变。
...
- Execution只能访问其绑定Account的Workspace；同一Agent的Memory可随Execution读取，但不能借此读取其他Account的Workspace
...
- Account系统管理：创建、删除、命名、默认/当前Agent、Workspace、接入Key生成/轮换/撤销
...
**全局设置入口**
- Appearance
- Account管理（创建Account、接入Key、默认/当前Agent、Workspace与接管审计；不作为Agent使用设置入口）
...
| Account系统管理 | `#/settings/accounts`、`#/settings/accounts/:accountId` | 新建/删除/命名Account，生成/轮换/撤销接入Key，查看默认/当前Agent、Workspace、Space与接管审计 | Agent Memory正文、Skills/Hooks/MCP/Data；provider/model表单；直接创建空Agent |
...
- 聊天时间线中Account头像/名称进入`#/settings/accounts/:accountId`；实际模型名可进入`#/agents/:executingAgentId`。代上线只改变模型名颜色，不改变Account头像、名称或Space成员。
