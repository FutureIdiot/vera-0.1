...
| 能力与数据设置 `[P5前端目录与内置binding；Phase 5.5/6通用runtime]` | 属于Agent使用层，不属于Vera全局Settings。Agent设置固定为Skills / Hooks / MCP / Data四个平级目录；Data首先承载Memory。四类都是独立资源，不嵌进Agent身份字段 |
...
- Agent系统管理：创建、删除、命名及其唯一Home Account绑定
- Account：来源、供应商、Key / 调用路径、模型名
- Vera全局Settings中的Account管理只负责上述系统层对象与连接关系；不得承载Agent当前状态、所在Space、Skills / Hooks / MCP / Data或Memory使用页

**Agent使用管理**
- Agent像素形象、当前状态与当前所在Space属于Agent使用层；像素形象现阶段是现有Agent视觉身份的展示，不因此新增第二套Avatar/Contact持久对象
- Agent使用管理固定提供Skills / Hooks / MCP / Data四个平级目录，Memory位于Data之下
- 同一页面有两个入口：聊天时间线中点击发言Agent的头像，直接定位该Agent；Space导航页左下角的联系人图标按钮按默认Agent顺序进入
- 两个入口复用同一路由和同一管理页，并都支持前后翻页。页面上半部分展示当前Agent像素形象，左右两侧是上一位/下一位箭头；头像入口以被点击Agent为当前页，联系人入口从默认顺序首位开始，其余翻页沿同一默认顺序
...
**全局设置入口**
- Appearance
- Account管理（Vera系统层的Agent生命周期、Home Account、Account连接与授权；不作为Agent使用设置入口）
...
| Space导航 | 聊天页左侧可折叠目录（`#/spaces`只作为可恢复的打开态深链） | 左侧Agent/群头像投影；右侧所选成员集合的Space目录；切换、新增、重命名、归档/恢复Space；左下角联系人图标进入Agent使用管理 | 固定选项或第二套持久状态；当前Space Seat/组件/提醒配置、Account连接配置；Contact实体或Contact CRUD |
...
| Account系统管理 | `#/settings/accounts`、`#/settings/accounts/:agentId` | Vera系统层管理Agent创建/删除/命名、唯一Home Account、连接、Workspace与授权策略 | Agent当前状态或所在Space；Skills/Hooks/MCP/Data；把授权账户展示成该Agent拥有的Account列表 |
| Agent使用管理 | `#/agents`、`#/agents/:agentId` | 两个入口复用同一页面；按默认顺序或指定Agent开始翻页；上半部分展示Agent像素形象并在左右提供前后翻页箭头；下半部分展示当前状态、所在Space及Skills/Hooks/MCP/Data四个平级入口 | Account连接、Key、Workspace、Agent创建/删除；把两个入口做成两套页面或两份状态 |
...
| Agent Data → Memory | `#/agents/:agentId/data/memory` | 以“Memory结构”展示active Provider，查看位置/状态、长期占用与待整理token压力；分别配置Digest和Dream执行Agent、任务模型、触发/调度，提供手动Digest与立即Dream，并进入长期Memory管理 | 显示或复制Recall/Write Hook binding；把跨Space总量说成下一轮精确消耗；展示未安装Provider假选项 |
...
- 联系人只存在于Space导航的展示层：单个Agent或Agent成员集合。左下角“联系人”按钮只是进入Agent使用管理的图标语义。允许组件使用 `contact-rail` 等UI命名，但store/API不得出现Contact CRUD或Contact持久记录。
- Account系统管理底层仍为 `Agent 1:1 Home Account`；其他Account只通过 `authorizedAgentIds` 授权具体subagent Execution使用，不成为该Agent的所有物。Memory与AgentSession归Agent，连接、Workspace与runtime归Account。
...
- 聊天时间线中，发言Agent的头像进入`#/agents/:agentId`并直接显示该Agent；Space导航左下角联系人图标进入`#/agents`，按默认Agent顺序显示第一位。两者必须复用同一分页状态模型，前后翻页沿同一默认顺序，不复制页面实现。
