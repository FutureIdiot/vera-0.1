...
| 能力与数据设置 `[Phase 5.5/6]` | Skills / MCP / Hooks / Agent Plugins 在Agent设置中各有独立通用目录；另设Data入口承载Memory等数据领域配置。两者都是独立资源，不嵌进Agent身份字段 |
...
**记忆整理流程：**
- Digest把尚未整理、已完整保存且对该Agent可见的Message转成长期Memory；Dream是对既有长期Memory做合并、纠错、归档、关系与派生权重维护的异步job。两者都不是普通聊天Run，Digest也不等于Dream
...
- 每个Agent的Data → Memory中，`digest.trigger.mode`只选择一种自动策略：`scheduled`与`realtime`互斥，`manual`表示关闭自动整理；无论选择哪种策略，owner与可信Agent都始终可以手动提交整理。`realtime`不是每轮都跑模型，而是按`(agentId, Space)`统计尚未整理、已完整保存的Message正文Unicode字符数，达到配置阈值后异步排队；Activity、流式delta、工具过程与provider token估算都不进入水位。聊天Run不等待整理。
...
- 待整理上下文按已完整保存且尚未Digest的Message统计；它不是供应商context window，也不是Digest后删除的短时缓存
...
Agent组合设置中，Skills / MCP / Hooks / Agent Plugins与基本信息、状态、来源/Runtime并列为独立通用能力目录；另设Data入口承载Memory及未来其他数据领域。Data → Memory只配置该Agent的active Provider、Provider位置/状态、待整理上下文、Digest、Dream与长期Memory管理，不复制MCP/Hook的安装和启用事实。MCP和Hook目录中的单元都展示启用开关；是否还需要执行Agent或其他控件由unit契约决定，不能给所有Hook强塞`executorAgentId`。gateway内置的Vera Memory MCP、Recall Hook和Write Hook均不展示执行Agent或模型。Digest/Dream的执行Agent与任务模型是Memory领域配置，只在Data → Memory中按真实执行资格候选展示，不扩张成所有MCP/Hook单元的字段。Skills沿用自身加载/卸载语义
...
- Agent设置的MCP目录必须把`Vera Memory MCP`（unit id=`vera.memory`）作为内置unit展示；它与第三方MCP共用启用开关外壳，但作为gateway第一方服务不展示执行Agent。不能借统一UI伪装成相同runtime。普通第三方MCP不会出现在Data → Memory的Provider候选；只有安装包中另行声明并通过`memory-provider`契约的unit才可被选择
...
| Account管理 / Agent设置 | `#/settings/accounts`、`#/settings/accounts/:agentId` | 组合展示Agent身份/状态、Home Account、授权策略及Skills/MCP/Hooks/Agent Plugins/Data入口 | 把Agent字段复制进Account；把授权账户展示成该Agent拥有的Account列表；在详情页预取所有子目录数据 |
| Agent Data → Memory | `#/settings/accounts/:agentId/data/memory` | 配置active Memory Provider、查看Provider位置/状态与待整理上下文、分别配置Digest/Dream执行Agent、任务模型和触发/调度、立即Dream，并进入长期Memory管理 | 复制MCP/Hook binding；把待整理量叫模型context缓存；展示未安装Provider假选项 |
| Agent Memory管理 | `#/settings/accounts/:agentId/memory` | 查看、编辑当前Agent active Provider支持的长期Memory；由Data → Memory按需进入 | 列表页预加载所有Agent的Memory正文；在Provider不支持编辑时伪造CRUD |
...
- 跨页面只传稳定ID和筛选条件。例如Agent详情先进入`#/settings/accounts/:agentId/data/memory`查看配置与状态，再按需进入`#/settings/accounts/:agentId/memory`管理长期Memory；两者复用同一Memory领域client，但不得把Provider配置、job状态和正文编辑塞回Account view或同一个巨型页面。
