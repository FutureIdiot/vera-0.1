...
必须与Key匹配，且token的`agentId`必须等于Account的`ownerAgentId`。非owner固定返回403
`delegation_unavailable`，不得读取Seat、Workspace或Account摘要；请求不接受`takeover/reason`。
owner已有会话或在飞Execution时重复登录返回409 `account_busy`。Key轮换后旧Key不能重连。
...
登出按owner Account会话进行；同一daemon同一时刻只允许这一Account会话。登出不改变Agent身份、Memory Provider placement或runtime。
