...
```js
{
  agent,          // Agent 对象（api-contract.md 形状，只含 id/name）。身份与记忆归它
  account,        // Account 对象（含 kind/provider/connection/model）。连接与会话上下文归它，
                  // adapter 取 connection/model 一律从 account。
  prompt: {
...
- 对 gateway **完全不透明**：一个可 JSON 序列化的对象，per (account, Space) 持久化（外部会话随 account 走，agent 在多个 Space 驾驶同一 account 即复用同一条外部会话；换驾驶员开新会话）。gateway 只做存取，**禁止读取其内部字段做任何逻辑**。
- 会话形态差异全部被它吸收：
  - daemon 型（OpenCode）：存外部会话 id，会话活在常驻服务里；
  - resume 型（Claude Code / Codex）：存 resume id，进程一次一命，靠 id 复活上下文。
- Space 重置会话（`/new` 类操作）= gateway 把该 (account, Space) 的 sessionState 清空，仅此而已。
...
- **secrets**：api 型 adapter 通过 `account.connection.secretRef` 向 `src/core/` 的 secrets 读取器换取明文，只存在于内存，不落日志、不进 sessionState。
...
- **编译层契约**（ground truth 2.3「群聊视角」）：gateway 侧的编译层对 CLI 与 API 输出同一份 `ctx.prompt.text`——它是"本轮应作为新 user 轮次投递的文本"，已含编译好的群内 delta（自该 agent 上次发言之后的他人气泡，以署名声告段形式）。adapter 自行把它翻译成协议帧：CLI 型直接作为新轮投递（外部 session 已有前文，本轮追加）；API 型自行拼接 system + 历史气泡（从 store 派生，本人+直接提问）+ 本轮 user。adapter **不得回头读 store 拼群状态**——所有需要的上下文已由 gateway 编译进 promptText，或来自 adapter 自身可选读取的 sessionState。这条让 adapter 各管各的协议形态、编译层保持薄。
...
| runner 参数 | `account.connection.args` 中只识别并透传 `--variant <v>`，其余成员一律不透传（防任意参数注入）；`-m <model>` 根据 `account.model`（空串则不传）|
