...
Agent daemon (加入同一 Tailscale tailnet 的 Mac / 另一台 VPS)
    ...
    │
    ▼
Vera gateway (可配置宿主，通常为小VPS 7×24)
  - 消息中枢 + 控制面持久化 (SpaceSession/AgentSession/API history/CLI binding/Message/Run/路由与绑定)
...
- **Execution与Run 1:1**：创建时固定`agentId + accountId + runtimeRevision + effectiveModel + modelVersion + delegated`。Account决定Space/Workspace/项目数据和当前模型选择，Agent决定Memory、provider/runtime以及可选模型列表。
- **Space主执行从Account解析Agent**：seat只含`accountId`；gateway使用该Account当前登录会话的`activeAgentId`创建Run。Account离线时不创建Run。
- **双凭证建立、Session续连**：Agent Token证明实际Agent身份，Account Key只在建立/重新建立Account授权时验证。成功后签发进程内Account Session Token；普通HTTP/SSE重连使用Agent Token + Session Token，不再重复发送Key。共享Account Key不得共享Agent Memory。
- **Account单活跃会话与租约**：同一Account只能有一个owner会话，且同一时刻最多一个running Execution。owner重复登录或竞争返回`account_busy`；Phase 5.5不存在takeover入口。
- **Account 1:1 Workspace，Agent持有runtime**：Workspace和当前模型选择随Account；provider连接、CLI/API生命周期、默认模型及`runtimeCapabilities.models`随Agent daemon。Account只能从所属Agent登记的列表中选择。AgentSession唯一键为`spaceSessionId + accountId + agentId`。
- **固定owner与Memory随Agent**：Account与owner Agent严格1:1；Phase 5.5只允许owner Agent登录自己的Account。Agent始终只读取自己的per-Agent Memory，不得读取另一Agent Memory或resume其provider binding/history。非owner执行只作为未来`vera.workspace` MCP闭环后的扩展方向。
...
Codex实现只服务`kind=cli, provider=codex`的Agent runtime，并同时实现聊天`run(ctx)`与隔离`digestMemory(...)`。聊天使用非交互`codex exec --json`，续轮使用`codex exec resume <threadId> --json`；prompt从stdin恰好投递一次，`thread.started.thread_id`立即作为当前`agentSessionId + generation + runtimeRevision`的CLI provider binding做CAS持久化。只有明确thread不存在才请求gateway换代generation并从checkpoint建立新thread；普通provider失败不得重置。provider binding不得跨Account或跨Agent复用。
...
因此，daemon是一个Agent的执行宿主。该Agent固定拥有一个owner Account，Phase 5.5只能登录该Account；取得其他Account的access key也不会获得会话。daemon、owner Workspace和默认CLI Memory Provider可位于PC或任一VPS，gateway可独立位于另一台小VPS，但当前Workspace必须与该daemon同`hostId`。

**Gateway 与 daemon 的职责切分（Phase 5.5目标形态）**：下表不描述Phase 2–5当前进程内承载，现状差异以附录A为准。

| 职责 | Gateway | Daemon |
|---|---|---|
| 控制面持久化（SpaceSession、AgentSession、API history/checkpoint、消息、Run、Activity、Approval、CLI binding、Provider/Workspace绑定） | ✅ | ❌ |
| 编译层（群聊视角 promptText） | ✅ | ❌（不回头读 store 拼群状态） |
| 触发判定（responseMode / 离线跳过 / blockAccountIds） | ✅ | ❌ |
| Account.presence / activeAgentId | ✅ 维护 | ✅ 以双凭证建立、进程内Session维持 |
...
| Vera Memory MCP server、身份校验、operation逻辑队列与Provider路由 | ✅ | ❌（daemon作为绑定agent身份的MCP client或CLI映射器） |
| active Memory Provider物理存储与原子提交 | placement为`gateway`时✅ | placement为`daemon`且hostId匹配时✅；`remote`由Provider服务承担 |
...
上表中的“MCP”默认指第三方或本机MCP；Vera Memory MCP按agent token绑定实际`agentId`。Account登录只允许自己的owner绑定，不能改写Memory MCP身份。gateway是Memory逻辑授权与校验权威，不等于所有Provider数据都物理位于gateway。

未来`vera.workspace` MCP也只是受目标Account Execution租约约束的Workspace工具平面。非owner Agent仍以自己的Agent Token、Memory和runtimeProfile执行，Account授权仍由目标Account Session决定；MCP不替换Agent身份、不改`ownerAgentId`，也不把目标Workspace或Account配置混入Agent profile。

Phase 5.5先建立gateway内的`Vera Control Service`作为上述权限判断的唯一入口。Control Service与gateway共用Account/Agent/Session/store，不部署第二套身份数据库；Workspace宿主通过第一方内部协议登记和取得逐Execution授权。当前只允许owner Agent及其同宿主Workspace，协议实现不得以MCP、SSH或共享Workspace Key作为前提。未来加入`vera.workspace`时只增加适配层，不改变Control Service的授权结果。
...
- compact target冻结`agentSessionId/fromGeneration/accountId/agentId/runtimeRevision/mode`并取得该Account租约；Phase 5.5只允许owner Agent处理，不得把任一Agent binding交给另一Agent。
...
`runtimeProfile`按API契约的版本化纯JSON形状登记，只是可便携、可稳定序列化的Agent配置。Account/owner归属、Workspace、`hostId`、session、presence、lease、token、Key、secret/`secretRef`、绝对路径及daemon派生的revision/capabilities/fingerprint/在线状态均不得进入该profile。
...
上面是建立/重新建立Account Session的模式。普通网络或SSE断线续连改用：

```http
POST /api/agent/login
Authorization: Bearer <agent-token>
X-Vera-Account-Session: <account-session-token>
```

两种header互斥；Key模式只在没有可用Session Token时使用，Session模式不得附带Account Key。
...
{
  "accountId": "acc_glm…",
  "daemonBootId": "boot_7f2d…",
  "runtime": {
    "hostId": "host_large_vps",
    "kind": "cli",
    "provider": "codex",
    "model": "gpt-5.6-sol",
    "revision": "sha256:…",
...
Bearer token固定实际`agentId`。Key模式以Account Key固定`accountId`并建立新Session；Session模式则要求
Token已绑定同一`agentId + accountId + daemonBootId`及当前gateway boot、Agent Token fingerprint、Key version。
Session缺失或任一绑定不符返回401 `account_reauthentication_required`，daemon再切换到Key模式。Key模式仍要求
`agentId === account.ownerAgentId`；非owner固定返回403 `delegation_unavailable`，不得读取Seat、Workspace或
Account摘要。请求不接受`takeover/reason`。

同一daemon与gateway进程存活期间，普通网络抖动、SSE重连或presence暂时offline都复用Session Token，不重验Key。
...
`runtime`描述实际Agent而不是Account。`runtime.model`是该daemon的默认模型；`runtimeCapabilities.models`是该Agent当前已接入、可供Account选择的精确模型列表，必须包含默认模型且不得含空值、`default`或重复项。gateway验证其与Agent已登记runtime profile兼容，
...
Workspace必须匹配Account绑定，且Phase 5.5要求`workspace.hostId === runtime.hostId`，否则返回409
`workspace_unavailable`。`memoryProvider`可选报告daemon placement的当前Provider revision；它必须匹配已保存
binding与同一`hostId`，不能在login时把数据静默迁到当前机器。gateway/remote placement不要求daemon承载存储。
runtime必须报告可在Run创建前解析出的非空默认模型和模型列表；前端只能选择列表成员。`run.requested`携带的冻结`effectiveModel`才是本次provider请求的模型，daemon必须验证它仍在自己的模型列表并把它传给adapter；不得用daemon默认模型覆盖Account选择。
...
// 200 响应
{
  ...
  "providerBindings": [
    {"agentSessionId":"ags_…","generation":1,"accountId":"acc_glm…","agentId":"agt_glm…","runtimeRevision":"sha256:…","version":"opaque","providerState":{"threadId":"…"}}
  ],
  "workspace": { … },
...
gateway只在owner登录会话期间把Agent runtime能力与自己Account的Workspace策略求交，形成该Account
...
| `stream.reset` | `{}` | 先用当前Session Token重新`POST /api/agent/login`拉齐；若返回`account_reauthentication_required`再用Account Key重新授权 |
...
每条Run就是一个Execution，`agentId/accountId/runtimeRevision/effectiveModel/modelVersion/delegated`创建后不可修改。主Run绑定seat Account与其当前active Agent；`effectiveModel/modelVersion`来自该Account创建Run时的模型选择快照。subagent只能沿用同pair、runtime revision与模型快照，不允许跨Account派生。
...
- **Execution 隔离**：main Run必须使用冻结的`spaceSessionId/agentSessionId/contextGeneration/accountId/agentId/runtimeRevision/effectiveModel/modelVersion`；Space/Workspace/项目权限和模型选择取Account，provider secret、runtime与模型候选取实际Agent，CLI binding取该AgentSession generation，API messages只取本Run payload。isolated subagent仅有审计用`spaceSessionId/accountId`，session字段为null，必须开临时provider上下文且终态销毁。不得因同属一个Agent混用Account或上下文。
...
- **secrets**：provider明文凭据只存在于实际Agent runtime可信内存，不落日志、不进AgentSession、history或provider binding；`secretRef=null`表示provider无鉴权。Account Key只用于enroll和Account Session建立/重新授权，可选保存在daemon本机`~/.vera/secrets.json`，但不进入runtime profile、provider请求、Run payload或gateway store。Account Session Token只在Key模式login成功响应中返回一次，此后只存在于daemon与gateway当前进程并经header发送；不落盘、不进日志、query、请求body或SSE data。
...
| 接口点 | 映射 |
|---|---|
| daemon 启动 | 生成新的`daemonBootId`，从Agent runtime配置解析OpenCode binary/provider/model → 健康check → 使用Account Key建立Account Session；同一进程后续断线只用Session Token续连 |
...
| 首轮聊天 | 在自己的Account Workspace启动`codex exec`，使用Run冻结的`effectiveModel`；stdin写入含可信Agent、Account与Space说明的完整`prompt.text` |
...
| 接口点 | 映射 |
|---|---|
| daemon 启动 | 生成新的`daemonBootId`并用Account Key建立Session，再由`POST /api/agent/login`拉回active AgentSessions的CLI bindings；普通断线用Session Token续连 |
...
| 接口点 | 映射 |
|---|---|
| daemon 启动 | 生成新的`daemonBootId`，校验Agent runtime为`kind=api, provider=ollama`并读取runtime `connection.baseUrl`；`secretRef`允许为null；再以Agent Token + Account Key建立Session，普通断线只用Session Token续连 |
