...
- **Memory随Agent**：Agent登录不同Account仍只读取自己的per-Agent Memory；同一Account换Agent时新Agent不得读取默认Agent Memory或resume其provider binding/history。
...
因此，daemon是一个Agent的执行宿主；它可以持有多个Account access key，但同一Account只允许一个Agent会话。登录Account会切换该Execution的Space/Workspace/项目上下文，不切换Agent Memory或模型身份。
...
- compact target冻结`agentSessionId/fromGeneration/accountId/agentId/runtimeRevision/mode`并取得该Account租约；只允许当前实际Agent处理，不得把默认Agentbinding交给代上线Agent。
...
body为`{accountId,agent:{name},runtimeProfile}`。成功创建稳定Agent身份，返回一次性
`{agent,agentToken,account}`；若Account的`defaultAgentId`为空则同时设为该Agent。
Account key可交给另一个未登记runtime做自己的Agent登记，但每次登记产生不同
`agentId/agentToken/Memory`，绝不共享默认Agent身份。
...
必须与Key匹配。`takeover:false`遇到其他`activeAgentId`返回409 `account_busy`；
当实际Agent不是`defaultAgentId`或`takeover:true`时都要求非空安全`reason`；显式接管时
...
`account.presence.updated`。同一daemon可依次登录多个Account，但每个Account是独立会话、
独立Workspace与独立租约。
...
每条Run就是一个Execution，`agentId/accountId/runtimeRevision/effectiveModel/delegated`创建后不可修改。主Run绑定seat Account与其当前active Agent。subagent默认沿用同pair与runtime revision；跨Account必须已有该Agent的独立有效Account会话。
...
登出按Account会话进行；同一daemon可登录多个Account，但每个Execution只使用其中一个不可变`accountId`。登录另一个Account不改变Agent身份、Memory或runtime。
