...
export function createMemoryOperations(deps) {
  ...
  async function applyOperation(operation) {
    ...
    if (!["user-api", "agent-mcp", "memory-hook", "external-scan"].includes(operation.origin)) {
      throw invalid("origin must be user-api, agent-mcp, memory-hook, or external-scan");
    }
    ...
    return queue.enqueue(agentId, async () => {
      ...
      if (kind === "create") return createMemory({ operation, path, snapshot });
      ...
      if (kind === "delete") {
        ...
        await scanAt(snapshot.root, agentId, { force: true, queueHeld: true });
        return null;
      }
      return updateMemoryFile({ operation, current, path, snapshot });
    });
  }

  async function createMemory({ operation, path, snapshot }) {
  ...
  async function updateMemoryFile({ operation, current, path, snapshot }) {
    const { agentId, kind, slug } = operation;
  ...
  return { applyOperation, saveMemory, getMemory, updateMemory, deleteMemory };
}
