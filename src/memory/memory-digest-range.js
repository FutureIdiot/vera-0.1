...
function isVisibleToAgent(message, agentId, accountId, blockedAccountIds) {
  if (message.author?.type === "account" && message.author.accountId === accountId &&
      message.executingAgentId === agentId) return true;
...
function orderedCompletedMessages(store, spaceId, spaceSessionId, agentId, accountId) {
  ...
  if (!account) throw invalid(`account ${accountId} does not exist`);
  const seat = space.seats?.find((candidate) => candidate.accountId === accountId);
  if (!seat) throw invalid(`account ${accountId} is not seated in Space ${spaceId}`);
...
export function resolveDigestRange({ store, agentId, accountId, spaceId, spaceSessionId, fromMessageId, toMessageId }) {
  ...
  if (typeof accountId !== "string" || !accountId) throw invalid("accountId is required");
  ...
  const visible = orderedCompletedMessages(store, spaceId, spaceSessionId, agentId, accountId);
...
export function resolveIncrementalDigestRange({ store, jobs = [], agentId, accountId, spaceId, spaceSessionId, toMessageId }) {
  if (typeof spaceSessionId !== "string" || !spaceSessionId) throw invalid("spaceSessionId is required");
  const completed = orderedCompletedMessages(store, spaceId, spaceSessionId, agentId, accountId);
  ...
  const succeededWatermarks = jobs
    .filter((job) => job.agentId === agentId && job.sourceAccountId === accountId &&
      job.spaceSessionId === spaceSessionId &&
