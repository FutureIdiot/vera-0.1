...
export async function run(ctx) {
  ...
  await check("p5-m2.5 Codex Account routes chat and digest while OpenCode digest stays paused", async () => {
    ...
    try {
      ...
      const reply = await waitForAgentReply(request, space.id, sleep, 5000);
      ...
      const job = await waitForJob(request, agent.id, queued.json.job.id, sleep, 5000);
      assertEqual(job.status, "succeeded");
      const memories = await request("GET", `/api/agents/${agent.id}/memory`);
      ...
      const openCode = await request("POST", "/api/agents", {
      ...
      const openSpace = await request("POST", "/api/spaces", {
        name: "Paused OpenCode", seats: [{ agentId: openCode.json.agent.id, responseMode: "silent" }],
      });
      const openMessage = await request("POST", `/api/spaces/${openSpace.json.space.id}/messages`, {
        author: { type: "user" }, target: { type: "broadcast" }, content: "Do not execute OpenCode digest.",
      });
      const openQueued = await request("POST", `/api/agents/${openCode.json.agent.id}/memory/_digest`, {
      ...
      const openJob = await waitForJob(request, openCode.json.agent.id, openQueued.json.job.id, sleep, 5000);
      assertEqual(openJob.status, "failed");
      assertEqual(openJob.error.code, "executor_unavailable");
